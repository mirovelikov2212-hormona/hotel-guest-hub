...
test("P4.1 builds a real version 1 dual-environment Product Factory blueprint", () => {
  assert.match(wizard, /version: 1/);
  assert.match(wizard, /production: true/);
  assert.match(wizard, /sandbox: true/);
  assert.match(wizard, /roomInventory/);
  assert.match(wizard, /departments: departments\.map/);
  assert.match(wizard, /integrations: \[\]/);
  assert.match(wizard, /workflows: \[\]/);
  assert.match(wizard, /services: \[\]/);
});
...
test("P4.1 supports range and explicit room inventories without a hotel-specific room assumption", () => {
  ...
  assert.match(wizard, /explicit: explicitRoomList\.map/);
...
test("P4.1 keeps timezone and locale input tenant-defined", () => {
  ...
  assert.match(wizard, /locales: localeList/);
...
test("P4.1 supports generic departments, hours, and after-hours targets", () => {
  assert.match(wizard, /hoursMode: "24h" \| "window"/);
  assert.match(wizard, /afterHoursDepartmentId/);
  assert.match(wizard, /setDepartments/);
  assert.match(wizard, /addDepartment/);
  assert.match(wizard, /removeDepartment/);
  assert.doesNotMatch(wizard, /housekeeping-default|maintenance-default/);
});

test("P4.1 does not collect or embed secrets", () => {
  assert.doesNotMatch(wizard, /password|api[_-]?key|access[_-]?token|client[_-]?secret/i);
  assert.match(route, /P2_FACTORY_SECRET_FORBIDDEN/);
});

test("P4.1 clearly defers services, workflows, and integrations before real onboarding", () => {
  assert.match(
    wizard,
    /P4\.2 ще добави services\/workflows\/integrations към този blueprint/,
  );
  assert.match(
    wizard,
    /P4\.2 will add services\/workflows\/integrations to this blueprint/,
  );
  assert.match(wizard, /P4\.1 не създава хотел/);
  assert.match(wizard, /P4\.1 does not create a hotel/);
});
...
test("P4.1 is wired into the full contract suite", () => {
  assert.match(
    ...
    /p4-1-onboarding-workspace-blueprint-wizard\.contract\.test\.mjs/,
  );
  assert.equal(
    pkg.scripts["test:p4-1"],
    "node --test tests/contracts/p4-1-onboarding-workspace-blueprint-wizard.contract.test.mjs",
  );
});
