...
export async function getHotelConfig(hotelSlug: string): Promise<HotelConfig | null> {
  ...
  const cfg: HotelConfig = {
    ...
    languages: ((pick(m, "languages", "bg,en,de").split(",").map((x) => x.trim()).filter(Boolean)) as LangKey[]) as any,
    languageDefault: (pick(m, "languageDefault", "bg") as LangKey) as any,
    opsLanguage: (pick(m, "opsLanguage", "bg") as LangKey) as any,
    staffHelperEnabled: pick(m, "staffHelperEnabled", "true").toLowerCase() !== "false",
    staffHelperLanguage: (pick(m, "staffHelperLanguage", "en") as LangKey) as any,
  };
