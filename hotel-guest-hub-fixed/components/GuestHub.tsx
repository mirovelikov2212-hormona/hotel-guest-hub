...
export default function GuestHub({ config }: { config: HotelConfig }) {
  ...
  }, [lang, tUI, lateCheckoutInfo, minibarNotice, wakeUpSlots]);
  ...
  function chooseWakeUpSlot() {
    if (!wakeUpSlots.length) return null;

    const list = wakeUpSlots
    ...
    if (wakeUpSlots.includes(picked)) return picked;

    const numericIndex = Number(picked);
    if (
      Number.isInteger(numericIndex) &&
      numericIndex >= 1 &&
      numericIndex <= wakeUpSlots.length
    ) {
      return wakeUpSlots[numericIndex - 1];
    }
