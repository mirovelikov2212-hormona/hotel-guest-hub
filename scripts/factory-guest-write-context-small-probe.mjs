...
function percentile(values, p) {
  ...
  return Number(
    sorted[Math.min(sorted.length - 1, Math.ceil((sorted.length * p) / 100) - 1)].toFixed(1),
  );
}
...
async function confirmIdentity(hotel) {
  ...
  const { response, body } = await fetchJson(`${baseUrl}/api/guest/stay/confirm`, {
    ...
    headers: {
      ...
      "x-stayhub-load-cell": "small-probe",
    },
    body: JSON.stringify({
      hotelSlug,
      room,
...
await writeFile(
  "factory-guest-write-context-small-probe-results.json",
