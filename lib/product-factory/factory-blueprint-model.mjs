import { validateFactoryDesignRuntime } from "./factory-design-runtime-model.mjs";
...
const DEPARTMENT_WEEKDAYS = Object.freeze(["mon", "tue", "wed", "thu", "fri", "sat", "sun"]);
const DEPARTMENT_WEEKDAY_SET = new Set(DEPARTMENT_WEEKDAYS);
...
function normalizeDepartmentCoverageWindows(hours, field, options = {}) {
  if (!Array.isArray(hours?.windows)) return null;
  const allowEmpty = options.allowEmpty === true;
  if (
    hours.windows.length > 64
    || (!allowEmpty && hours.windows.length < 1)
  ) {
    throw new Error(`P0_FACTORY_INVALID:${field}.hours.windows`);
  }

  return hours.windows.map((window, index) => {
    const windowField = `${field}.hours.windows.${index}`;
    if (!window || typeof window !== "object" || Array.isArray(window)) {
      throw new Error(`P0_FACTORY_INVALID:${windowField}`);
    }

    const days = Array.isArray(window.days)
      ? [...new Set(window.days.map((day) => String(day || "").trim().toLowerCase()))]
      : [];
    if (
      days.length < 1
      || days.length > 7
      || days.some((day) => !DEPARTMENT_WEEKDAY_SET.has(day))
    ) {
      throw new Error(`P0_FACTORY_INVALID:${windowField}.days`);
    }

    const open = normalizeClockTime(window.open, `${windowField}.open`);
    const close = normalizeClockTime(window.close, `${windowField}.close`);
    if (!open || !close || open === close) {
      throw new Error(`P0_FACTORY_INVALID:${windowField}`);
    }

    const label = normalizeOptionalText(window.label);
    return {
      days: DEPARTMENT_WEEKDAYS.filter((day) => days.includes(day)),
      open,
      close,
      ...(label ? { label } : {}),
    };
...
function normalizeSeasonCoverageRule(rawSeason, field) {
  if (!rawSeason || typeof rawSeason !== "object" || Array.isArray(rawSeason)) {
    throw new Error(`P0_FACTORY_INVALID:${field}`);
  }

  const id = requireString(rawSeason.id, `${field}.id`);
  if (id.length > 120) throw new Error(`P0_FACTORY_INVALID:${field}.id`);

  const startDate = normalizeCalendarDate(rawSeason.startDate, `${field}.startDate`);
  const endDate = normalizeCalendarDate(rawSeason.endDate, `${field}.endDate`);
  if (startDate > endDate) throw new Error(`P0_FACTORY_INVALID:${field}.dateRange`);

  const windowsRaw = Array.isArray(rawSeason.windows) ? rawSeason.windows : [];
  if (windowsRaw.length > 64) throw new Error(`P0_FACTORY_INVALID:${field}.windows`);

  if (rawSeason.is24h === true) {
    if (windowsRaw.length) throw new Error(`P0_FACTORY_INVALID:${field}.windows`);
    return {
      id,
      startDate,
      endDate,
      is24h: true,
      windows: [],
      ...(normalizeOptionalText(rawSeason.label) ? { label: normalizeOptionalText(rawSeason.label) } : {}),
    };
  }

  const windows = windowsRaw.map((window, index) => {
    const windowField = `${field}.windows.${index}`;
    if (!window || typeof window !== "object" || Array.isArray(window)) {
      throw new Error(`P0_FACTORY_INVALID:${windowField}`);
    }
    const days = Array.isArray(window.days)
...
function normalizeDepartmentSeasons(hours, field) {
  if (hours?.seasons === undefined) return [];
  if (!Array.isArray(hours.seasons) || hours.seasons.length > 32) {
    throw new Error(`P0_FACTORY_INVALID:${field}.hours.seasons`);
  }

  const seasons = hours.seasons.map((season, index) =>
    normalizeSeasonCoverageRule(season, `${field}.hours.seasons.${index}`),
  );
  ...
  for (let index = 0; index < seasons.length; index += 1) {
    ...
    if (ids.has(season.id)) {
      throw new Error(`P0_FACTORY_DUPLICATE:${field}.hours.seasons.id:${season.id}`);
    }
    ids.add(season.id);

    for (let previous = 0; previous < index; previous += 1) {
      const other = seasons[previous];
      const overlap =
        season.startDate <= other.endDate
        && other.startDate <= season.endDate;
      if (overlap) {
        throw new Error(`P0_FACTORY_INVALID:${field}.hours.seasons.overlap`);
      }
    }
  }
...
function normalizeDepartmentDateOverrides(hours, field) {
  if (hours?.dateOverrides === undefined) return [];
  if (!Array.isArray(hours.dateOverrides) || hours.dateOverrides.length > 366) {
    throw new Error(`P0_FACTORY_INVALID:${field}.hours.dateOverrides`);
  }

  const seenDates = new Set();
  return hours.dateOverrides.map((override, index) => {
    const overrideField = `${field}.hours.dateOverrides.${index}`;
    if (!override || typeof override !== "object" || Array.isArray(override)) {
      throw new Error(`P0_FACTORY_INVALID:${overrideField}`);
    }

    const date = normalizeCalendarDate(override.date, `${overrideField}.date`);
    if (seenDates.has(date)) {
      throw new Error(`P0_FACTORY_DUPLICATE:${field}.hours.dateOverrides.date:${date}`);
    }
    seenDates.add(date);

    const mode = String(override.mode || "").trim().toLowerCase();
    if (!["closed", "24h", "custom"].includes(mode)) {
      throw new Error(`P0_FACTORY_INVALID:${overrideField}.mode`);
    }

    const windowsRaw = Array.isArray(override.windows) ? override.windows : [];
    if (mode === "closed" || mode === "24h") {
      if (windowsRaw.length) throw new Error(`P0_FACTORY_INVALID:${overrideField}.windows`);
      return {
        date,
        mode,
        windows: [],
        ...(normalizeOptionalText(override.label) ? { label: normalizeOptionalText(override.label) } : {}),
      };
    }

    if (windowsRaw.length < 1 || windowsRaw.length > 16) {
      throw new Error(`P0_FACTORY_INVALID:${overrideField}.windows`);
    }

    const windows = windowsRaw.map((window, windowIndex) => {
      const windowField = `${overrideField}.windows.${windowIndex}`;
      if (!window || typeof window !== "object" || Array.isArray(window)) {
        throw new Error(`P0_FACTORY_INVALID:${windowField}`);
      }
      const open = normalizeClockTime(window.open, `${windowField}.open`);
...
export function normalizeFactoryDepartment(department, field = "department") {
  ...
  if (hours !== undefined && hours !== null) {
    ...
    coverageSeasons = normalizeDepartmentSeasons(hours, field);
    coverageDateOverrides = normalizeDepartmentDateOverrides(hours, field);

    const hasWindows = Array.isArray(hours.windows);
    if (hours.is24h === true) {
      if (hours.open || hours.close || (hasWindows && hours.windows.length)) {
        throw new Error(`P0_FACTORY_INVALID:${field}.hours`);
      }
      is24h = true;
    } else if (hasWindows) {
      ...
      coverageWindows = normalizeDepartmentCoverageWindows(hours, field, {
        allowEmpty: coverageSeasons.length > 0 || coverageDateOverrides.length > 0,
      });
      if (
        coverageWindows.length === 0
        && coverageSeasons.length === 0
        && coverageDateOverrides.length === 0
      ) {
        throw new Error(`P0_FACTORY_INVALID:${field}.hours.windows`);
      }
    } else {
...
export function validateFactoryBlueprint(blueprint) {
  ...
  if (blueprint.designRuntime !== undefined) {
    const designValidation = validateFactoryDesignRuntime(blueprint.designRuntime);
    if (!designValidation.ok) {
      throw new Error(`P0_FACTORY_INVALID:designRuntime:${designValidation.errors.join(",")}`);
    }
    if (
      !blueprint.designHandoff
      || blueprint.designHandoff.authority !== "exact_immutable_design_revision"
    ) {
      throw new Error("P0_FACTORY_INVALID:designRuntime.designHandoff");
    }
  }
