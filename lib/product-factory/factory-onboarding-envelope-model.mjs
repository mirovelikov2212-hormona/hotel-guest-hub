...
import { prepareFactoryGuestRuntimeConfig } from "./factory-guest-runtime-config-model.mjs";
...
export function prepareFactoryOnboardingEnvelope({ blueprint }) {
  ...
  const guestRuntime = prepareFactoryGuestRuntimeConfig({ blueprint });
  ...
  const branding = {
    ...
    logo_url: null,
    icon_url: null,
    theme: designRuntime ? jsonClone(designRuntime.theme) : {},
    source_design_revision_id: designRuntime?.sourceDesignRevisionId || null,
    source_design_revision_checksum: designRuntime?.sourceDesignRevisionChecksum || null,
  };
  ...
  const envelope = {
    ...
    health,
    guest_runtime: {
      schema_version: guestRuntime.schemaVersion,
      status: guestRuntime.status,
      config_hash: guestRuntime.configHash,
      config: jsonClone(guestRuntime.config),
      counts: jsonClone(guestRuntime.counts),
    },
  };
  ...
  return {
    ...
    guestRuntimeHash: guestRuntime.configHash,
    counts: {
      ...
      guestRuntimeRooms: guestRuntime.counts.rooms,
      guestRuntimeRequestDefs: guestRuntime.counts.requestDefs,
      guestRuntimeVisibleRequestDefs: guestRuntime.counts.guestVisibleRequestDefs,
      guestRuntimeOffers: guestRuntime.counts.offers,
      guestRuntimeDesignAssets: guestRuntime.counts.designAssets,
    },
  };
}
