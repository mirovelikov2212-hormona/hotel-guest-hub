...
function normalizeRuntimeOffer(offer) {
  ...
  const readyCreativeByLang = {};
  const rawReadyCreative = assets.readyCreativeByLang && typeof assets.readyCreativeByLang === "object" && !Array.isArray(assets.readyCreativeByLang)
    ? assets.readyCreativeByLang
    : {};
  for (const [language, creative] of Object.entries(rawReadyCreative)) {
    if (!creative || typeof creative !== "object" || Array.isArray(creative)) {
      throw new Error("FACTORY_DESIGN_OFFER_READY_CREATIVE_INVALID");
    }
    const assetId = text(creative.assetId).toLowerCase();
    const kind = text(creative.kind).toLowerCase();
    if (!["default","bg","en","de","ro","cs","ru"].includes(language) || !UUID.test(assetId) || !["image","document"].includes(kind)) {
      throw new Error("FACTORY_DESIGN_OFFER_READY_CREATIVE_INVALID");
    }
    readyCreativeByLang[language] = { assetId, kind };
  }
  const presentationMode = text(offer.presentationMode || "structured").toLowerCase();
  if (!["structured","ready_asset"].includes(presentationMode)) {
    throw new Error("FACTORY_DESIGN_OFFER_PRESENTATION_INVALID");
  }
  if (presentationMode === "ready_asset" && Object.keys(readyCreativeByLang).length < 1) {
    throw new Error("FACTORY_DESIGN_OFFER_READY_CREATIVE_REQUIRED");
  }
  if (
  ...
  return {
    ...
    assets: { coverAssetId, galleryAssetIds, attachmentAssetIds, readyCreativeByLang },
    presentationMode,
    status,
...
export function prepareFactoryDesignRuntime(input) {
  ...
  const assetIds = uniqueStrings(
    offers.flatMap((offer) => [
      ...
      ...Object.values(offer.assets.readyCreativeByLang || {}).map((creative) => creative.assetId),
    ]),
