...
function normalizeRuntimeServicePrice(value, serviceKey) {
  if (value === null || value === undefined || value === "") return null;
  const text = String(value).trim().replace(",", ".");
  if (!/^\d+(?:\.\d{1,2})?$/.test(text)) {
    throw new Error(`P4_12_SERVICE_PRICE_INVALID:${serviceKey}`);
  }
...
function normalizeRuntimeServiceCurrency(value, serviceKey) {
  if (value === null || value === undefined || value === "") return null;
  const currency = String(value).trim().toUpperCase();
  if (!/^[A-Z]{3}$/.test(currency)) {
    throw new Error(`P4_12_SERVICE_CURRENCY_INVALID:${serviceKey}`);
  }
  return currency;
}
...
function departmentSchedules(coreDepartments) {
  const mapWeeklyWindows = (windows) => (
    Array.isArray(windows)
      ? windows.map((window) => ({
          days: [...window.days],
          open: String(window.open).slice(0, 5),
          close: String(window.close).slice(0, 5),
          ...(window.label ? { label: String(window.label) } : {}),
        }))
      : []
  );
  ...
  const mapSeasons = (seasons) => (
    Array.isArray(seasons)
      ? seasons.map((season) => ({
          ...
          is24h: season.is24h === true,
          windows: mapWeeklyWindows(season.windows),
          ...(season.label ? { label: String(season.label) } : {}),
        }))
      : []
  );

  const mapDateOverrides = (overrides) => (
  ...
  for (const department of coreDepartments) {
    ...
    const seasons = mapSeasons(department.coverage_seasons);
    const dateOverrides = mapDateOverrides(department.coverage_date_overrides);
    ...
    if (department.is_24h === true) {
      result[department.code] = {
        ...
        ...(dateOverrides.length ? { dateOverrides } : {}),
      };
      continue;
    }

    const windows = mapWeeklyWindows(department.coverage_windows);
    if (windows.length) {
      result[department.code] = {
        is24h: false,
        windows,
    ...
    if (seasons.length || dateOverrides.length) {
      result[department.code] = {
        is24h: false,
        windows: [],
        ...(seasons.length ? { seasons } : {}),
...
function materializeRequestDefs({ operationalResources, coreResources, locales }) {
  ...
  return operationalResources.services.map((service, index) => {
    ...
    const price = normalizeRuntimeServicePrice(
      definition.price ?? standard?.price,
      service.key,
    );
    const currency = normalizeRuntimeServiceCurrency(
      definition.currency ?? standard?.currency,
      service.key,
    );
    if ((price === null) !== (currency === null)) {
      throw new Error(`P4_12_SERVICE_PRICE_CURRENCY_PAIR_REQUIRED:${service.key}`);
    }
...
export function prepareFactoryGuestRuntimeConfig({ blueprint }) {
  ...
  const designRuntime = normalizedBlueprint.designRuntime?.schemaVersion === "factory-design-runtime-v1"
    ? normalizedBlueprint.designRuntime
    : null;
  ...
  const config = {
    ...
    theme: designRuntime ? { ...designRuntime.theme } : {},
    offers: designRuntime ? designRuntime.offers.map((offer) => structuredClone(offer)) : [],
    designAssetSourceKey: designRuntime?.assetSourceKey || null,
    designRevision: designRuntime
      ? {
          revisionId: designRuntime.sourceDesignRevisionId,
          checksum: designRuntime.sourceDesignRevisionChecksum,
        }
      : null,
    contacts: builtinContacts(normalizedBlueprint.departments),
    departmentHours: departmentHours(coreResources.departments),
    departmentSchedules: departmentSchedules(coreResources.departments),
    housekeepingExtras: [],
    taxiProviders: [],
    reviews: {},
    socialLinks: [],
  ...
  return {
    ...
    counts: {
      ...
      departmentsWithSchedules: Object.keys(config.departmentSchedules).length,
      hotelInfoItems: hotelInfoItems.length,
      venues: venueRows.length,
      offers: config.offers.length,
      designAssets: designRuntime?.assetIds.length || 0,
    },
  };
}
