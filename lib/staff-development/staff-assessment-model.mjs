  ...
  const questionResults = assessment.questions.map((question) => {
    ...
    if (question.type === "free_text") {
      ...
      if (answerText.length > 12000) {
        throw new Error("STAFF_ASSESSMENT_FREE_TEXT_TOO_LONG");
      }
...
export function assertStaffAssessmentAttemptIntegrity(attemptInput) {
  if (!isRecord(attemptInput)) {
    throw new Error("STAFF_ASSESSMENT_ATTEMPT_INVALID");
  }
  const resultHash = normalizeHash(
    attemptInput.resultHash,
    "STAFF_ASSESSMENT_RESULT_HASH_INVALID",
  );
  const { resultHash: _ignored, ...core } = attemptInput;
  if (sha256(core) !== resultHash) {
    throw new Error("STAFF_ASSESSMENT_RESULT_HASH_MISMATCH");
  }
  return structuredClone(attemptInput);
}
...
export function assertVerifiedStaffAssessmentResult(resultInput) {
  ...
  return {
    ...
    scorePercent: Number(
      resultInput.scorePercent ?? resultInput.autoScorePercent,
    ),
  };
}
