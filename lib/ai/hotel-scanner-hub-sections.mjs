...
const POLICY_ATTRIBUTES = new Set([
  ...
  "quiet_hours", "noise_policy", "noise_penalty", "cancellation_policy", "payment_policy", "dress_code", "age_policy",
...
const SERVICE_ENTITY_PATTERN = /(?:kids?\s*(?:corner|club)|children'?s?\s*(?:corner|club)|детски\s*(?:кът|клуб)|hairdress|barber|beauty\s*salon|pharmacy|drugstore|laundry|магазин|фризьор|бербер|салон|дрогерия|аптек|пералн|паркинг|parking|transfer|shuttle)/iu;
const WELLNESS_ENTITY_PATTERN = /(?:\bspa\b|wellness|medical|therapy|treatment|massage|hydrotherapy|physiotherapy|kinesiotherapy|спа|уелнес|медиц|терап|масаж|хидротерап|физиотерап|кинезитерап)/iu;
...
export function hotelScannerHubSectionForFact(fact = {}) {
  ...
  if (category === "wellness" || WELLNESS_ATTRIBUTES.has(attribute) || WELLNESS_ENTITY_PATTERN.test(all)) return "wellness";

  // Known guest services/amenities always remain Services even if an AI pass labels
  // them as experiences (kids corner, barber, pharmacy, transfer, etc.).
  if (SERVICE_ENTITY_PATTERN.test(entityText)) return "services";

  // Explicit activity/attraction semantics outrank a generic `services` category.
  // This keeps real things-to-do (for example Rose Valley) in Experiences while the
  // service-entity override above still keeps hotel amenities in Services.
