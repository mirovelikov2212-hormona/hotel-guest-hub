...
export function selectCriticalHotelScannerPages(pages, options = {}) {
  ...
  // actually present. This prevents room/marketing volume from crowding out
  // FAQ, policy, operational, venue, SPA or service-detail evidence.
  for (const rule of CATEGORY_RULES) {
    if (selected.length >= maxPages) break;
    const representative = relevant.find((item) => item.categories.includes(rule.key));
    add(representative);
  }
