...
function polarity(value, attribute) {
  ...
  if (attribute === "pet_policy") {
    if (/(?:no pets?|not allowed|not permitted|forbidden|prohibit|не (?:се|са) (?:допуск\p{L}*|разреш\p{L}*|прием\p{L}*)|забран|keine\s+haustiere|haustiere.*(?:nicht\s+(?:erlaubt|gestattet)|verboten))/iu.test(candidate)) return "prohibited";
...
function normalizeAddressValue(value) {
  ...
  result = result.replace(/ß/giu, "ss");
  result = result.replace(/\b(?:a|at|d|de|ch|it|fr|es|pt|nl|be|cz|sk|pl|hu|ro|bg|hr|si|rs)-(?=\d{4,6}\b)/giu, "");
  for (const [pattern, replacement] of COUNTRY_ALIASES) result = result.replace(pattern, replacement);
  result = result.replace(/\bmevkii\b/giu, "mevki");
...
function normalizeClaimSemantics(fact) {
  ...
  if (attribute === "address") {
    ...
    const localityLabel = /^(?:city(?:\s*(?:and|&)\s*region)?|town|locality|region(?:\s+(?:(?:of|for)\s+(?:hotels?|properties|operations?|activity)|of\s+operation))?|operating\s+(?:region|area)|(?:region|area)\s+(?:of\s+)?(?:operation|activity)|province|state|district(?:\s*(?:and|&)\s*province)?|град(?:\s+и\s+регион)?|ра[ий]он(?:\s+(?:(?:и\s+)?провинция|на\s+де[ий]ност(?:та)?))?|област(?:\s+(?:и\s+)?провинция)?|провинция(?:\s+(?:и\s+)?област)?|регион(?:\s+на\s+(?:хотелите|хотели|де[ий]ност(?:та)?))?|населено място|[sş]ehir|il[cç]e|il|faaliyet\s+b[oö]lgesi|stadt(?:\s+und\s+region)?|bezirk(?:\s+und\s+provinz)?|ort|provinz|betriebsregion|t[aä]tigkeitsregion|ora[sș]|jude[tț]|regiune(?:\s+de\s+(?:hoteluri|operare))?|m[eě]sto|kraj|oblast\s+p[uů]sobnosti|город|ра[ий]он|область)$/iu.test(label);
