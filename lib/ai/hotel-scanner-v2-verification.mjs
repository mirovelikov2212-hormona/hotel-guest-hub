...
function normalizeClaimSemantics(fact) {
  ...
  if (attribute === "address") {
    ...
    const localityLabel = /^(?:city(?:\s*(?:and|&)\s*region)?|town|locality|region(?:\s+(?:of|for)\s+(?:hotels?|properties))?|province|state|district(?:\s*(?:and|&)\s*province)?|град(?:\s+и\s+регион)?|ра[ий]он(?:\s+(?:и\s+)?провинция)?|област(?:\s+(?:и\s+)?провинция)?|провинция(?:\s+(?:и\s+)?област)?|регион(?:\s+на\s+(?:хотелите|хотели))?|населено място|[sş]ehir|il[cç]e|il|stadt(?:\s+und\s+region)?|bezirk(?:\s+und\s+provinz)?|ort|provinz|ora[sș]|jude[tț]|regiune(?:\s+de\s+hoteluri)?|m[eě]sto|kraj|город|ра[ий]он|область)$/iu.test(label);
