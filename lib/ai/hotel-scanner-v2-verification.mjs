...
function polarity(value, attribute) {
  ...
  if (attribute === "pet_policy") {
    if (/(?:no pets?|not allowed|not permitted|forbidden|prohibit|не (?:се|са) (?:допуск\p{L}*|разреш\p{L}*|прием\p{L}*)|забран|(?:keine\s+haustiere|haustiere.*(?:nicht\s+(?:erlaubt|gestattet)|verboten)|nicht\s+(?:erlaubt|gestattet)))/iu.test(candidate)) return "prohibited";
...
function normalizeClaimSemantics(fact) {
  ...
  if (attribute === "address") {
    ...
    const localityAtom = "(?:city|town|locality|region|province|state|district|град|област|регион|населено място|[sş]ehir|il[cç]e|il|stadt|ort|provinz|ora[sș]|jude[tț]|regiune|m[eě]sto|kraj|город|область)";
    const localityLabel = new RegExp(
      `^(?:${localityAtom})(?:\\s*(?:and|&|и|und|ve|și|si|a)\\s*(?:${localityAtom}))?import { hotelScannerSourceDocumentKey, verifyHotelScanFacts } from "./hotel-scanner-verification.mjs";
...
function normalizeClaimSemantics(fact) {
  ...
  if (attribute === "address") {
    ...
    const postalLabel = /(?:postal(?: code)?|post ?code|zip(?: code)?|пощенски код|posta kodu|postleitzahl|cod po[sș]tal|ps[cč]|почтовый индекс)/iu.test(label);
      ...
      + `|^(?:region|регион|regiune)\\s+(?:of|for|на|за|der|des|de)\\s+(?:hotels?|properties|хотелите|хотели|отелите|oteller|hoteluri)import { hotelScannerSourceDocumentKey, verifyHotelScanFacts } from "./hotel-scanner-verification.mjs";
