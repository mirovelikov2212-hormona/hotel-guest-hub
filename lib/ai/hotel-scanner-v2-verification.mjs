...
function normalizeClaimSemantics(fact) {
  ...
  if (attribute === "address") {
    ...
    const localityLabel = /^(?:city(?:\s*(?:and|&)\s*region)?|town|locality|region(?:\s+(?:of|for)\s+(?:hotels?|properties))?|province|state|district|град(?:\s+и\s+регион)?|област|регион(?:\s+на\s+(?:хотелите|хотели))?|населено място|[sş]ehir|il[cç]e|il|stadt(?:\s+und\s+region)?|ort|provinz|ora[sș]|jude[tț]|regiune(?:\s+de\s+hoteluri)?|m[eě]sto|kraj|город|область)$/iu.test(label);
