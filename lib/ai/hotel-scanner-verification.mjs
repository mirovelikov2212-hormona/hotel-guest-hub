...
function clockTokens(value) {
  ...
  const tokens = [];
  const seen = new Set();
  const regex = /(?:^|\D)([01]?\d|2[0-3])[:.]([0-5]\d)\s*(a\.?m\.?|p\.?m\.?)?(?!\d)/giu;
  let match;
...
function policyPolarity(value, kind) {
  ...
  if (kind === "pet_policy") {
    if (/(?:no pets?|pets? (?:are )?not (?:allowed|permitted|accepted)|not allowed|not permitted|forbidden|prohibit|не (?:се|са) (?:допуск\p{L}*|разреш\p{L}*|прием\p{L}*)|домашни любимци не|забран|(?:keine\s+haustiere|haustiere.*(?:nicht\s+(?:erlaubt|gestattet)|verboten)|nicht\s+(?:erlaubt|gestattet)))/iu.test(candidate)) return "prohibited";
