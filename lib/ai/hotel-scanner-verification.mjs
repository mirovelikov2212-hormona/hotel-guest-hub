...
function policyPolarity(value, kind) {
  ...
  if (kind === "pet_policy") {
    if (/(?:no pets?|pets? (?:are )?not (?:allowed|permitted|accepted)|not allowed|not permitted|forbidden|prohibit|не (?:се|са) (?:допуск\p{L}*|разреш\p{L}*|прием\p{L}*)|домашни любимци не|забран|keine\s+haustiere|haustiere.*(?:nicht\s+(?:erlaubt|gestattet)|verboten))/iu.test(candidate)) return "prohibited";
