...
const NON_ACTIONABLE_POLICY_GAP = /(?:(?:polic(?:y|ies)|rules?|политик(?:а|и|ите)?|правил(?:а|ата)?).*(?:not (?:specified|stated|provided|found)|missing|не (?:са|е) (?:посочен|описан|предоставен|намерен))|(?:не (?:са|е) предоставен[иа]).*(?:правил|политик))/iu;
...
export function attachHotelScanConflictReview(profile, conflicts, outputLanguage = "en") {
  ...
  const existing = (Array.isArray(nextProfile.uncertainties) ? nextProfile.uncertainties : [])
    .filter((note) => !NON_ACTIONABLE_POLICY_GAP.test(text(note, 1_000)));
