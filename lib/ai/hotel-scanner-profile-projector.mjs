...
function acceptedFact(fact) {
  const status = String(fact?.verification?.status || "");
  return status !== "CONFLICT" && Number(fact?.confidence ?? 0) >= 0.8 && text(fact?.value);
}

function factsFor(profile, predicate) {
  return (Array.isArray(profile?.facts) ? profile.facts : []).filter((fact) => acceptedFact(fact) && predicate(fact));
}
...
function projectVenues(profile) {
  ...
  const diningFacts = factsFor(profile, (fact) => category(fact) === "dining");
  const grouped = new Map();
  for (const fact of diningFacts) {
  ...
  for (const [key, entry] of grouped.entries()) {
    ...
    byName.set(key, {
      ...
      hours: current.hours || (hours ? value(hours) : ""),
      summary: current.summary || (summary ? value(summary) : ""),
    });
  }
...
export function projectVerifiedHotelScanFacts(inputProfile) {
  ...
  const amenities = factsFor(profile, (fact) => category(fact) === "amenities" && ["amenity", "facility", "service"].includes(attribute(fact))).map(value);
  ...
  const policies = factsFor(profile, (fact) => category(fact) === "policy").map((fact) => `${text(fact.label, 120)}: ${value(fact)}`);
  ...
  profile.hospitality.policies = unique([...(profile.hospitality.policies || []), ...policies], 40);
