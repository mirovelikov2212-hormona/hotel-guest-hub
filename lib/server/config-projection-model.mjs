...
export function resolveProjectionRuntimeRoute(definition) {
  ...
  // Factory-managed revisions already carry the exact operational service type
  // and target department. Preserve that immutable meaning instead of collapsing
  // the service into a legacy generic category. This makes normal projection and
  // automatic reconciliation the same deterministic path.
  if (definition.factoryManagedGuestRuntime === true) {
    if (!authoritativeDepartment) return null;
    return {
      request_type: sourceRequestType,
      department_code: authoritativeDepartment,
      after_hours_department_code:
        authoritativeDepartment === "housekeeping" ||
        authoritativeDepartment === "maintenance"
          ? "reception"
          : null,
  ...
  // Legacy configurations keep their historical canonical aliases so existing
  // hotels do not change behavior as the Factory runtime becomes authoritative.
...
export function buildHotelConfigProjection(config) {
  ...
  requestDefinitions.forEach((rawDefinition, index) => {
    ...
    if (!route) {
      if (
        rawDefinition.factoryManagedGuestRuntime === true &&
        !RUNTIME_AUTHORITY_DEPARTMENTS.has(
          normalizeKey(rawDefinition.targetDepartment),
        )
      ) {
        errors.push(`FACTORY_ROUTING_DEPARTMENT_UNSUPPORTED:${index}`);
      } else {
