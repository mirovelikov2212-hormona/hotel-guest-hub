...
const HOURS_KEYS = new Set(["departmentHours", "departmentSchedules"]);
...
function fingerprintChangeValue(present, value) {
  return sha256(
    present
      ? { present: true, value }
      : { present: false },
...
function appendChange(
  ...
  changes.push({
    ...
    beforeHash: fingerprintChangeValue(beforePresent, beforeValue),
    afterHash: fingerprintChangeValue(afterPresent, afterValue),
  });
}
...
function diffArrays(left, right, path, changes) {
  ...
  if (!leftMap || !rightMap) {
    if (!collectionSemanticallyEqual(left, right)) {
      appendChange(changes, path, "collection_changed", {
        beforeValue: left,
        afterValue: right,
      });
    }
  ...
  for (const identity of identities) {
    ...
    if (!hasLeft) {
      appendChange(changes, itemPath, "added", {
        beforePresent: false,
        afterValue: rightMap.get(identity),
      });
      continue;
    }
    if (!hasRight) {
      appendChange(changes, itemPath, "removed", {
        beforeValue: leftMap.get(identity),
        afterPresent: false,
      });
      continue;
    }
...
export function buildHotelConfigVersionDiff(currentConfig, candidateConfig) {
  ...
  const evidence = {
    ...
    valueBindingVersion: "sha256-canonical-v1",
