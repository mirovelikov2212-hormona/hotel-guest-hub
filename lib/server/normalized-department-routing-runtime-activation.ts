...
import { buildSandboxNormalizedDepartmentRoutingRuntimeConfig } from "@/lib/server/normalized-config-runtime-model.mjs";
import {
  getActiveNormalizedDepartmentRoutingRows,
...
export async function setSandboxNormalizedDepartmentRoutingReads(input: {
  hotelSlug: string;
  enabled: boolean;
  actor?: string | null;
}) {
  ...
  if (input.enabled) {
    const rows = await getActiveNormalizedDepartmentRoutingRows(hotel.id);
    ...
    const validation =
      buildSandboxNormalizedDepartmentRoutingRuntimeConfig({
        isSandbox: true,
        hotelTimeZone: String(hotel.timezone || ""),
  ...
  const metadata = {
    ...
    runtimeDepartmentRoutingReadsActivation: {
      status: input.enabled ? "enabled" : "disabled",
      actor: String(
        input.actor || "internal_config_department_routing_runtime_reads",
      ).slice(0, 200),
      changedAt: now,
  ...
  if (error) {
    return {
      ok: false,
      status: 500,
      error: "DEPARTMENT_ROUTING_RUNTIME_READ_ACTIVATION_UPDATE_FAILED",
      hotelId: hotel.id,
      hotelSlug: hotel.slug,
    };
  }
