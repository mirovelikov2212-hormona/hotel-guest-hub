...
const ROOM_HEADING = /(?:\broom\b|\bsuite\b|\bstudio\b|\bapartment\b|\bvilla\b|\bchalet\b|\bzimmer\b|\bappartement\b|\bcamer[ăa]\b|\bapartament\b|\bpokoj\b|\bapartm[aá]n\b|\bстая\b|\bстудио\b|\bапартамент\b|\bномер\b)/iu;
const ROOM_METRIC_HEADING = /^(?:\d+(?:[.,]\d+)?\s*(?:m²|m2|sq\.?\s*m|sqm)\b|from\s+[€$£]?\s*\d|[€$£]\s*\d|ab\s+\d|от\s+\d)/iu;
const ROOM_ACTION_HEADING = /^(?:compare\s+rooms?|room\s+compare|room\s+comparison|compare|details?|view\s+(?:room|suite)|book(?:\s+now)?|reserve|zimmer\s*vergleich|zimmervergleich|сравни\s+стаи|сравнение\s+стаи)$/iu;
const ROOM_DETAIL_LINK = /\/(?:rooms?|room-details?|zimmer|zimmer-detail|suites?|apartments?|appartements?|studios?|villas?|chalets?)(?:\/|$)/iu;
...
const DINING_ENTITY_NAME = /(?:restaurant|bar|cafe|café|bistro|club|grill|lounge|tavern|brasserie|dining|ресторант|бар|кафе|клуб|бистро|грил|лаундж|gaststätte|restaurant|bar|café|club|grill|lounge|restaurante?|baruri|cafenea|club|grătar|restaurace|bary|kavárna|klub|gril|ресторан|бар|кафе|клуб)/iu;
const DINING_HOTEL_SEO_HEADING = /hotel/iu;
...
const DINING_OPERATIONAL = /(?:opening\s+hours?|opening\s+times?|öffnungszeiten|oeffnungszeiten|dress\s*code|dresscode|reserv(?:ation|ierung)|book\s+a\s+table|order|bestellen|room\s+service|guestnet|breakfast\s*:|dinner\s*:|lunch\s*:|frühstück\s*:|fruehstueck\s*:)/iu;
...
function candidateEvidence(block, domain, pagePrimaryDomain) {
  ...
  if (domain === "accommodation") {
    ...
    const linkedDetail = (Array.isArray(block?.links) ? block.links : []).some((href) => ROOM_DETAIL_LINK.test(String(href || "")));
    if (ROOM_METRIC_HEADING.test(heading) || ROOM_ACTION_HEADING.test(heading)) return null;
    if (!semantic && !linkedDetail) return null;
    return {
      score: (semantic ? 8 : 0) + (linkedDetail ? 8 : 0) + (metric ? 2 : 0) + (section ? 4 : 0),
      section,
    };
  }
  ...
  if (domain === "gastronomy") {
    ...
    if (DINING_HOTEL_SEO_HEADING.test(heading) && !explicitVenueName) return null;
    if (!semantic && !operational && !explicitVenueName) return null;
    if (!explicitVenueName && !operational && (DINING_THEME_HEADING.test(heading) || words >= 7)) return null;
    if (!explicitVenueName && !operational && words > 4) return null;
    return {
      score: (operational ? 10 : 0) + (explicitVenueName ? 5 : 0) + (semantic ? 2 : 0) + (section ? 4 : 0),
      section,
    };
  }
