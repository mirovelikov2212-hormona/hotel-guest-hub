...
const CONTENT_PATTERNS = Object.freeze({
  ...
  dining: /(?:\brestaurant(?:s)?\b|\bdining\b|\blobby bar\b|\brestaurant bar\b|\bресторант|\bгастроном|\блоби бар)/iu,
...
export function planHotelScannerSecondaryUrls(input = {}) {
  ...
  if (maxPages === 0) {
    return {
      ...
      underCorroboratedDomains: COVERAGE_DOMAINS.filter((domain) => counts[domain.key] < domain.targetDepth).map((domain) => domain.key),
    };
  }
