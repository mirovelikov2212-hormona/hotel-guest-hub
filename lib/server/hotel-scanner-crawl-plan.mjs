const COVERAGE_DOMAINS = Object.freeze([
  ...
  { key: "dining", weight: 12, targetDepth: 5, pattern: /(?:restaurant|restaurants|bar|bars|dining|food|drink|gastronomy|ресторант|бар|хран|напит|гастроном)/iu },
  ...
  { key: "services", weight: 10, targetDepth: 4, pattern: /(?:service|services|facility|facilities|amenit|услуг|удобств)/iu },
...
const SENSITIVE_QUERY_KEYS = new Set([
  "token", "access_token", "auth", "authorization", "session", "sessionid", "sid", "code", "state",
  "email", "user", "userid", "user_id", "guest", "guestid", "guest_id", "member", "account",
  "bookingid", "booking_id", "reservationid", "reservation_id", "confirmation", "payment", "password",
]);
...
export function isPublicBusinessCrawlUrl(rawUrl, canonicalOrigin = "") {
  try {
    ...
    if (canonicalOrigin && url.origin !== canonicalOrigin) return false;
    if (url.username || url.password) return false;
    if (PERSONAL_OR_PRIVATE_PATH.test(url.pathname) || PRIVATE_FILE_PATH.test(url.pathname)) return false;
    for (const key of url.searchParams.keys()) {
      if (SENSITIVE_QUERY_KEYS.has(String(key).toLocaleLowerCase("en-US"))) return false;
    }
    return true;
  } catch {
    return false;
...
export function classifyHotelScannerUrlCoverage(rawUrl) {
  ...
  return COVERAGE_DOMAINS.filter((domain) => domain.pattern.test(haystack)).map((domain) => domain.key);
}
...
function legacyPriority(rawUrl) {
  ...
  const signals = [
    ...
    "terms", "booking", "gallery", "price", "menu",
  ];
...
function candidateRecord(url, originalIndex) {
  ...
  return { url, originalIndex, domains, legacyPriority: legacyPriority(url), lowValue: LOW_VALUE_PATH.test(url) };
}

function coverageCounts(input) {
  const counts = Object.fromEntries(COVERAGE_DOMAINS.map((domain) => [domain.key, 0]));
  const supplied = input?.domainVisitCounts && typeof input.domainVisitCounts === "object" ? input.domainVisitCounts : {};
  for (const domain of COVERAGE_DOMAINS) {
    const value = Number(supplied[domain.key] || 0);
    if (Number.isFinite(value) && value > 0) counts[domain.key] = Math.floor(value);
  }
  for (const key of canonicalDomainKeys(input?.alreadyCoveredDomains || [])) counts[key] = Math.max(1, counts[key]);
  return counts;
}
...
function coverageDiagnostics(counts) {
  return {
    ...
    underCorroboratedDomains: COVERAGE_DOMAINS.filter((domain) => counts[domain.key] > 0 && counts[domain.key] < domain.targetDepth).map((domain) => domain.key),
  };
}
...
export function planHotelScannerSecondaryUrls(input = {}) {
  ...
  for (const [index, raw] of uniqueUrls(input.links || [], 600).entries()) {
    try {
      ...
      if (!isPublicBusinessCrawlUrl(url.toString(), canonicalOrigin)) continue;
      url.hash = "";
