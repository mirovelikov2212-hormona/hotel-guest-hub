...
function activeProjectionDepartmentRouting(projection) {
  return {
...
function normalizedActiveDepartmentRouting(rows) {
  return {
    schema_version: "m10.2",
    departments: sortDepartments(
...
function buildRoomRelationalAuthority(rows, input) {
  const roomIdByNumber = {};

  for (const row of Array.isArray(rows?.rooms) ? rows.rooms : []) {
    if (row?.active !== true) continue;
    const roomNumber = normalizeText(row?.room_number).replace(/\s+/g, "");
    const roomId = normalizeUuid(row?.id);
    if (!roomNumber || !roomId || roomIdByNumber[roomNumber]) return null;
    roomIdByNumber[roomNumber] = roomId;
  }

  if (!Object.keys(roomIdByNumber).length) return null;

  return {
    revisionId: normalizeText(input?.publishedRevisionId),
    sourceChecksum: normalizeChecksum(input?.publishedChecksum),
    roomIdByNumber,
  };
}

function buildDepartmentRelationalAuthority(rows, input) {
  const departmentIdByCode = {};
  const routingDepartmentIdByRequestType = {};

  for (const row of Array.isArray(rows?.departments) ? rows.departments : []) {
    if (row?.active !== true) continue;
    const code = normalizeText(row?.code).toLowerCase();
    const departmentId = normalizeUuid(row?.id);
    if (!code || !departmentId || departmentIdByCode[code]) return null;
    departmentIdByCode[code] = departmentId;
  }

  for (const row of Array.isArray(rows?.routingRules) ? rows.routingRules : []) {
    if (row?.active !== true) continue;
    const requestType = normalizeText(row?.request_type).toLowerCase();
    const departmentCode = normalizeText(row?.department_code).toLowerCase();
    const departmentId = normalizeUuid(row?.department_id);
    if (
      !requestType ||
      !departmentCode ||
      !departmentId ||
      routingDepartmentIdByRequestType[requestType] ||
      departmentIdByCode[departmentCode] !== departmentId
    ) {
      return null;
    }
  ...
  if (
    !Object.keys(departmentIdByCode).length ||
    !Object.keys(routingDepartmentIdByRequestType).length
  ) {
    return null;
  }
...
function hasMatchingDepartmentRoutingCounts(state, counts) {
  return (
...
function mergeNormalizedDepartmentRoutingAuthority(
  config,
  projection,
  hotelTimeZone,
) {
  ...
  const requestDefs = (Array.isArray(config.requestDefs)
    ...
    return {
      ...
      afterHoursDepartment:
        normalizedRule.after_hours_department_code || "none",
    };
  });
  ...
  return {
    ...
    departmentHours,
    requestDefs,
    hotelTimezone: normalizeText(hotelTimeZone),
...
export function buildSandboxNormalizedRoomRuntimeConfig(input) {
  ...
  return {
    ...
    relationalAuthority: buildRoomRelationalAuthority(input?.rows, input),
  };
}
...
export function buildSandboxNormalizedDepartmentRoutingRuntimeConfig(input) {
  ...
  const metadata = isObject(state.metadata_json) ? state.metadata_json : {};
  if (metadata.runtimeDepartmentRoutingReadsActivated !== true) {
    return fallback(
      publishedConfig,
  ...
  if (!isValidTimeZone(input?.hotelTimeZone)) {
    return fallback(publishedConfig, "HOTEL_TIME_ZONE_INVALID");
  }
  ...
  if (!hasMatchingDepartmentRoutingCounts(state, expected.counts)) {
    return fallback(publishedConfig, "PROJECTION_STATE_COUNT_MISMATCH");
  }

  const expectedActive = activeProjectionDepartmentRouting(expected.projection);
  const normalizedActive = normalizedActiveDepartmentRouting(input?.rows);
  ...
  return {
    ...
    source: "normalized",
    reason: null,
    config: mergeNormalizedDepartmentRoutingAuthority(
      publishedConfig,
      normalizedActive,
      input.hotelTimeZone,
    ),
    relationalAuthority: buildDepartmentRelationalAuthority(input?.rows, input),
  };
}
