...
const INVENTORY_LANDING_TYPES = new Set([
  "accommodation",
  "gastronomy",
  "spa",
  "services",
...
const NON_CONTENT_PATH = /(?:^|\/)(?:wp-admin|wp-login|login|account|cart|checkout|book|booking|booking-engine|book-now|reservation-engine|search|tag|author|feed|cdn-cgi)(?:\/|$)|(?:^|\/)(?:add|remove)(?:\/|$)/iu;
const NAVIGATION_LEGAL_CHROME_PATH = /(?:^|\/)(?:privacy|privacy-policy|datenschutz|gdpr|cookie-policy|cookies?|impressum)(?:\/|$)/iu;
...
function addCandidate(map, rawUrl, source, page = null) {
  ...
  if (source === "navigation") {
    try {
      if (NAVIGATION_LEGAL_CHROME_PATH.test(new URL(url).pathname)) return;
    } catch {
      return;
    }
  }
...
function markLogicalVariantsCovered(fetchedKeys, page) {
  const variants = [
    page?.canonicalHint,
    ...(Array.isArray(page?.languageAlternates) ? page.languageAlternates.map((item) => item?.url) : []),
  ];
  for (const raw of variants) {
    const url = normalizeUrl(raw);
    if (!url || !isCandidateUrl(url)) continue;
    fetchedKeys.add(semanticPathKey(url));
  }
}
 ...
 *
 * Language alternates/canonical siblings of a fetched content page count as the
 * same covered logical source. Policy/FAQ disagreements remain independently
 * discoverable because their distinct semantic paths are not collapsed here.
 *
...
export function buildHotelScannerCoveragePlanV2(input = {}) {
  ...
  // A successful request counts as read coverage even when the server redirects
  // it onto a page already retained in `pages`. Without this, the requested
  // alias can remain permanently pending although its response was read.
  for (const key of attemptedKeys) {
    if (!failedKeys.has(key)) fetchedKeys.add(key);
  }
  ...
  for (const page of pages) {
    ...
    const key = semanticPathKey(url);
    fetchedKeys.add(key);
    markLogicalVariantsCovered(fetchedKeys, page);
    const result = classification(page);
    if (!isRelevantClassification(result)) continue;
    fetchedRelevant.set(key, { key, url, type: result.type, domain: result.domain, score: Number(TYPE_PRIORITY[result.type] || TYPE_PRIORITY.other) + 300, sources: new Set(["fetched"]) });
    ...
    const contentLinks = Array.isArray(page?.contentLinks)
      ...
      : Array.isArray(page?.links)
        ? page.links
        : [];
    const inheritsInventoryAuthority = INVENTORY_LANDING_TYPES.has(result.type);
    for (const link of contentLinks) {
      const child = classification({ url: link });
      if (inheritsInventoryAuthority) {
        // Opaque entity slugs (for example /superior-room or /nero) are valid
        // inventory candidates when an authoritative domain landing links them.
        addCandidate(candidates, link, "relevant_parent_link");
      } else if (isRelevantClassification(child)) {
        // Detail pages often contain editorial/recommendation cards. They may
        // discover another already-typed hotel surface, but cannot promote an
        // otherwise opaque URL into required coverage.
        addCandidate(candidates, link, "internal_link");
      }
    }
  ...
  const relevantLogical = new Map(candidates);
  for (const [key, entry] of fetchedRelevant) {
    if (!relevantLogical.has(key)) relevantLogical.set(key, entry);
  }

  const discoveredRelevant = [...relevantLogical.values()]
    .filter((entry) => isRelevantClassification(entry) || entry.sources.has("relevant_parent_link") || entry.sources.has("fetched"))
    .sort((left, right) => right.score - left.score || left.url.localeCompare(right.url));

  const covered = discoveredRelevant.filter((entry) => fetchedKeys.has(entry.key));
  ...
  return {
    ...
    fetchedRelevantCount: covered.length,
    ...
    fetchedRelevantUrls: covered.map((entry) => entry.url).sort(),
