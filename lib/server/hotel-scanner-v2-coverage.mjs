...
const NON_HUB_LEGAL_PATH = /(?:^|\/)(?:privacy(?:-policy)?|cookie(?:s|-policy)?|gdpr|imprint|impressum|legal-notice|terms-of-use|data-protection|datenschutz|politika-za-poveritelnost|politica-de-confidentialitate|polityka-prywatnosci|politica-de-privacidad|politique-de-confidentialite)(?:\/|$)/iu;
...
function isCandidateUrl(raw) {
  try {
    ...
    return /^https?:$/i.test(url.protocol)
      && !NON_CONTENT_PATH.test(decodedPath)
      // Site privacy/cookie/legal chrome is not hotel operating policy for the Guest Hub.
      // Do not exclude broad hotel terms/conditions here because they may contain
      // check-in, cancellation, payment, pet or other guest-operational rules.
      && !NON_HUB_LEGAL_PATH.test(decodedPath)
      && !NON_HTML_EXTENSION.test(decodedPath)
      && !/\.pdf$/iu.test(decodedPath);
  } catch {
    return false;
  }
}
