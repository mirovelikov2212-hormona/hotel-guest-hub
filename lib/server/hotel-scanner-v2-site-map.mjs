...
import { deriveHotelPageInventoryHintsV2 } from "./hotel-scanner-v2-landing-inventory.mjs";
...
const VOLATILE_BOOKING_QUERY_KEYS = new Set([
  "adults", "adult", "children", "child", "kids", "infants", "pets", "pet",
  "checkin", "check-in", "checkout", "check-out", "arrival", "departure",
  "rooms", "room", "occupancy", "guests", "nights", "promo", "promocode", "coupon", "currency",
]);
const LANGUAGE_SEGMENT = /^(?:bg|en|de|ro|ru|cs|cz|fr|it|es|pl|tr|el|sr|mk|uk|hu|nl|pt)$/iu;
const PDF_PATH = /\.pdf$/iu;
const GENERIC_OFFER_LANDING_SEGMENT = /^(?:offers?|special-offers?|packages?|promotions?|angebote|sonderangebote|teklifler?|kampanyalar?|paketler?|firsatlar?|fırsatlar?|oferte|promotii|promoții|nabidky|nabídky|оферти|пакети|промоции)$/iu;
...
export function canonicalizeHotelIntakeUrl(rawUrl, baseUrl = "") {
  try {
    ...
    for (const [key, value] of url.searchParams.entries()) {
      ...
      if (TRACKING_QUERY_KEYS.has(normalizedKey) || VOLATILE_BOOKING_QUERY_KEYS.has(normalizedKey)) continue;
...
function stableOfferLandingGroupId(url) {
  try {
    const parsed = new URL(url);
    const segments = parsed.pathname.split("/").filter(Boolean)
      .filter((segment) => !LANGUAGE_SEGMENT.test(segment));
    if (!segments.length || !GENERIC_OFFER_LANDING_SEGMENT.test(segments[segments.length - 1])) return "";
    segments[segments.length - 1] = "offers";
    return `${parsed.hostname}/${segments.join("/")}`.toLocaleLowerCase("en-US");
  } catch {
    return "";
  }
}
...
export function buildHotelSiteMapV2(evidence = {}) {
  ...
  for (const page of pages) {
    ...
    if (page?.delegatedAuthority?.domain === "offers" && page?.delegatedAuthority?.sourceUrl) {
      addResource(resources, pageUrl, canonicalUrl, {
        kind: "delegated_offer_detail",
        sourceUrl: page.delegatedAuthority.sourceUrl,
        language: page?.language,
      }, { crawled: true });
      relations.push({
        kind: "delegated_offer_detail",
        fromUrl: canonicalizeHotelIntakeUrl(page.delegatedAuthority.sourceUrl, canonicalUrl),
        toUrl: pageUrl,
      });
    }
  ...
  const outputResources = [...resources.values()].map((resource) => {
    ...
    const classification = resource.resourceType === "pdf"
      ...
      : page?.delegatedAuthority?.domain === "offers"
        ? { primaryType: "offer_detail", types: ["offer_detail", "offers"], confidence: 1, signals: ["delegated_offer_detail"] }
        : classifyHotelScannerPageV2(page || { url: resource.url });
    const inventoryHints = page ? deriveHotelPageInventoryHintsV2(page, classification) : [];
    const inventoryHint = inventoryHints[0] || null;
    const structuralInventory = page ? deriveHotelStructuralInventoryV2(page, classification) : null;
    return {
      ...resource,
      ...
      inventoryHints,
      structuralInventory,
      contactSignals: page?.contactSignals || { phones: [], emails: [], addresses: [] },
      delegatedAuthority: page?.delegatedAuthority || null,
      variantGroupId: classification.primaryType === "offers"
        ? stableOfferLandingGroupId(resource.url) || groupIdFor(resource.url)
        : groupIdFor(resource.url),
    };
