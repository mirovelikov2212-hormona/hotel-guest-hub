...
function candidateScore(group, parent) {
  ...
  const visit = (parent) => {
    ...
    if (children.length >= 2) {
      ...
      for (const child of children) {
        ...
        if (stats.nodes < 2 || stats.anchors < 1) continue;
      ...
      for (const entry of byShape.values()) {
        ...
        const confidence = candidateScore(entry.values, parent);
        ...
        for (let index = 0; index < entry.values.length && items.length < MAX_ITEMS_PER_GROUP; index += 1) {
          ...
          items.push({
            url: link.url,
            label: primaryLabel(item) || link.label,
            ordinal: index,
