...
function candidateScore(group, parent, baseUrl) {
  ...
  if (["ul", "ol"].includes(parent?.tag) && uniqueLabels >= Math.max(2, Math.ceil(count * 0.6))) score += 0.15;
  ...
  const visit = (parent) => {
    ...
    if (children.length >= 2) {
      ...
      for (const child of children) {
        ...
        const inlineListLeaf = child.tag === "li" && stats.nodes >= 1;
        if (stats.nodes < 2 && !inlineListLeaf) continue;
