...
export function deriveHotelPageInventoryHintsV2(page = {}, classification = {}) {
  ...
  // Some resort CMSes, however, publish a gastronomy overview under a detail-like
  // SEO route. When that page exposes multiple independent culinary blocks, keep
  // those blocks as a bounded inventory surface.
  if (isDetail) {
    if (primaryDomain !== "gastronomy") return [];
    const culinaryBlocks = uniqueCandidates(blockCandidates.get("gastronomy") || []);
