...
const COUNT_NOUNS = Object.freeze({
  ...
  gastronomy: /(?:dining\s+venues?|venues?|restaurants?|bars?|culinary\s+(?:venues?|outlets?)|gastronomic\s+(?:venues?|experiences?)|кулинарни\s+обекта?|гастрономически\s+обекта?|ресторанти?|барове?|gastronomische\s+(?:erlebnisse|betriebe)|lokale|restaurante?|baruri|restaurace|bary|ресторанов?|баров?)/iu,
...
const CANDIDATE_PATTERNS = Object.freeze({
  accommodation: /(?:\broom\b|\bsuite\b|\bstudio\b|\bapartment\b|\bzimmer\b|\bappartement\b|\bcamer[ăa]\b|\bapartament\b|\bpokoj\b|\bapartm[aá]n\b|\bстая\b|\bстудио\b|\bапартамент\b|\bномер\b)/iu,
  gastronomy: /(?:\brestaurant\b|\bbar\b|\bcafe\b|\bcafé\b|\bbistro\b|\bdining club\b|\bресторант\b|\bбар\b|\bкафе\b|\bgastst[aä]tte\b|\brestaurante?\b|\brestaurace\b|\bресторан\b)/iu,
  services: /(?:kids? corner|children'?s corner|game hall|games room|playroom|hair salon|barbershop|pharmacy|fitness (?:centre|center)|conference hall|parking|transfer|laundry|business centre|детски кът|игрална зала|фитнес|фризьор|аптека|конферентна зала|паркинг|трансфер|пералн)/iu,
  experiences: /(?:corner|game hall|games room|salon|barbershop|pharmacy|concept store|fitness (?:centre|center)|valley|historical route|route|landmark|museum|park|tourism|trail|hiking|culture|tradition|excursion|attraction|детски кът|игрална зала|салон|аптека|магазин|фитнес|долина|маршрут|забележителност|музей|парк|туризъм|пътека|култура|традиц)/iu,
});
...
function exactCountCluster(headings, expectedCount) {
  ...
  for (const heading of headings) { if (!byLevel.has(heading.level)) byLevel.set(heading.level, []); byLevel.get(heading.level).push(heading.text); }
  return [...byLevel.entries()].filter(([, names]) => names.length === expectedCount).sort(([a], [b]) => b - a)[0]?.[1] || [];
}
function lexicalCandidates(domain, headings) {
  const pattern = CANDIDATE_PATTERNS[domain]; if (!pattern) return [];
  return headings.filter((heading) => pattern.test(heading.text)).map((heading) => heading.text);
}
function uniqueNames(values) {
  const seen = new Set(); const result = [];
  for (const raw of values) { const name = normalizedName(raw); const key = name.toLocaleLowerCase("en-US"); if (!name || seen.has(key)) continue; seen.add(key); result.push(name); }
  return result;
}
