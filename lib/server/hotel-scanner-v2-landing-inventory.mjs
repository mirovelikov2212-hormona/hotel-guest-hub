...
const RULES = Object.freeze({
  ...
  gastronomy: {
    entityType: "venue",
    heading: /(?:restaurant|restoran|lokanta|bar|cafe|café|kafe|bistro|dining|in[- ]?room\s+dining|room\s+service|cuisine|culinary|kulinarik|genuss|grill|lounge|ресторант|бар|кафе|gastst[aä]tte|restaurante?|restaurace|ресторан)/iu,
...
function isGastronomyOverviewDetail(page) {
  const title = normalizedName(page?.title).split(/\s+(?:[-–—|])\s+/u)[0] || "";
  return GASTRONOMY_OVERVIEW_DETAIL_TITLE.test(title);
}

function gastronomyTextCandidates(page, primaryDomain) {
  if (primaryDomain !== "gastronomy") return [];
  const headings = (Array.isArray(page?.headings) ? page.headings : []).map((item) => normalizedName(item?.text)).filter(Boolean);
  const blocks = inventoryEvidenceBlocks(page);
  const source = [
    clean(page?.text, 30_000),
    ...headings,
    ...blocks.flatMap((block) => [normalizedName(block?.heading), clean(block?.text, 4_000)]),
  ].join(" ");
  const result = [];
  if (/(?:^|\s)in[- ]?room\s+dining(?:\s|$)/iu.test(source)) {
    result.push({
      domain: "gastronomy",
      name: "IN-ROOM DINING",
      entityType: "venue",
      basis: "deterministic_gastronomy_text",
      score: 12,
      links: [],
    });
  } else if (/(?:^|\s)room\s+service(?:\s|$)/iu.test(source)) {
    result.push({
      domain: "gastronomy",
      name: "Room Service",
      entityType: "venue",
...
function inventoryEvidenceBlocks(page) {
  const rendered = Array.isArray(page?.renderedContentBlocks) ? page.renderedContentBlocks : [];
  const html = Array.isArray(page?.contentBlocks) ? page.contentBlocks : [];
  const seen = new Set();
  const result = [];
  for (const block of [...rendered, ...html]) {
    const heading = normalizedName(block?.heading);
    if (!heading) continue;
    const sectionPath = (Array.isArray(block?.sectionPath) ? block.sectionPath : [])
      .map((value) => normalizedName(value).toLocaleLowerCase("en-US"))
      .filter(Boolean)
      .join(">");
    const fingerprint = `${Number(block?.level || 0)}|${sectionPath}|${heading.toLocaleLowerCase("en-US")}`;
...
function scoreBlock(block, domain, primaryDomain) {
  ...
  const gastronomyRoomService = domain === "gastronomy"
    && primaryDomain === "gastronomy"
    && /(?:in[- ]?room\s+dining|room\s+service)/iu.test(heading);
  if (commonObject && commonObject.domain !== domain && !gastronomyRoomService) return null;
...
function offerDetailLinkCandidates(page, primaryDomain) {
  ...
  const result = [];
  for (const block of inventoryEvidenceBlocks(page)) {
...
export function deriveHotelPageInventoryHintsV2(page = {}, classification = {}) {
  ...
  const isDetail = primaryType.endsWith("_detail");
  const blockCandidates = assignSemanticBlocks(page, primaryDomain);
  const jsonLdCandidates = assignJsonLd(page, primaryDomain);
  const gastronomyText = gastronomyTextCandidates(page, primaryDomain);

  // Detail pages normally describe one object and must not become mini-landings.
  // Only a clearly generic culinary overview/SEO route may expose multiple
  // independent Hub components. Named restaurant/bar detail pages remain atomic.
  if (isDetail) {
    if (primaryDomain === "gastronomy" && isGastronomyOverviewDetail(page)) {
      const culinaryBlocks = uniqueCandidates([
        ...(blockCandidates.get("gastronomy") || []),
        ...gastronomyText,
      ]);
      if (culinaryBlocks.length < 2) return [];
      const hint = buildHint(page, "gastronomy", "__detail_container__", culinaryBlocks);
      return hint ? [hint] : [];
    }
    if (primaryDomain === "experiences") {
      const experienceBlocks = uniqueCandidates(blockCandidates.get("experiences") || []);
      if (experienceBlocks.length < 2) return [];
      const hint = buildHint(page, "experiences", "__detail_container__", experienceBlocks);
      return hint ? [hint] : [];
    }
    return [];
  }
  ...
  if (gastronomyText.length) domains.add("gastronomy");
  ...
  for (const domain of domains) {
    const candidates = domain === "offers" && authoritativeOfferLinks.length
      ...
      : [
          ...
          ...(domain === "gastronomy" ? gastronomyText : []),
