...
// Large crawl graphs are projected only for persistence; discovery and
// validation continue to operate on the complete in-memory graph.
...
function retainResourceClosure(siteMap, seeds) {
  ...
  // Language alternates and backlink sources are intentionally not expanded
  // here. If they were crawled/relevant they are already seeds; otherwise
  // retaining them would recreate the oversized graph this projection removes.
  const structuralKinds = new Set(["canonical", "document_link"]);
...
function compactResource(resource, retainedUrls) {
  const {
    inventoryHint: _inventoryHint,
    inventoryHints: _inventoryHints,
    ...rest
  } = resource || {};
  return {
    ...rest,
...
function compactRelations(relations, retainedUrls) {
  ...
  for (const relation of relations || []) {
    ...
    const fromUrl = cleanUrl(relation?.fromUrl);
    const toUrl = cleanUrl(relation?.toUrl);
    if (!fromUrl || !toUrl || !retainedUrls.has(fromUrl) || !retainedUrls.has(toUrl)) continue;

    const key = `${relation.kind}|${fromUrl}|${toUrl}`;
    if (seen.has(key)) continue;
    seen.add(key);
    result.push({ ...relation, fromUrl, toUrl });
  }
