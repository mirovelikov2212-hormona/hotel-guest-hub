...
function resolveEffectiveDepartment({ hotelConfig, primaryDepartment, afterHoursDepartment, now }) {
  const hasConfiguredHours = hasConfiguredDepartmentScheduleForConfig({
    hotelConfig,
...
export function resolveOperationalWorkflow(input) {
  ...
  if (clean(authority.sourceRequestDef) !== sourceRequestDef) {
    return notApplicable("OPERATIONAL_AUTHORITY_SOURCE_MISMATCH");
  }

  const primaryDepartment = clean(authority.department);
  if (!primaryDepartment) {
    return notApplicable("OPERATIONAL_AUTHORITY_DEPARTMENT_UNAVAILABLE");
  }
