...
function unique(values) {
  return Array.from(new Set((Array.isArray(values) ? values : []).map((value) => clean(value).toLowerCase()).filter(Boolean)));
}
...
export function resolveOperationalWorkflow(input) {
  ...
  const requestedFields = unique(routerResult.requested_fields);
  if (!requestedFields.includes("request")) {
    return notApplicable("OPERATIONAL_REQUEST_NOT_REQUESTED");
  }

  const selectedIds = unique(routerResult.selected_ids);
  ...
  return {
    ...
    action: {
      ...
      notifyDepartments: unique(authority.notifyDepartments),
