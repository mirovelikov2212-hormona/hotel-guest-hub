...
const EXPECTED_VERCEL_TEAM_ID =
  process.env.VERCEL_TEAM_ID || "team_3XjEa4JGz8DWSWIW3RqauJYA";
const EXPECTED_VERCEL_PROJECT_ID =
  process.env.VERCEL_PROJECT_ID || "prj_KUkOL6tRgwxr0QD9tc1TVClCdf9Y";
...
async function hasActiveSandboxCanaryTraffic() {
  ...
  const promise = (async () => {
    const { data, error } = await supabaseAdmin.rpc(
      "has_active_sandbox_canary_traffic_v1",
    );
...
export async function resolveSandboxCanaryRoute(hotel: HotelRoutingScope) {
  ...
  const { data, error } = await supabaseAdmin.rpc(
    "resolve_guest_sandbox_canary_route_v1",
    { p_hotel_id: hotel.id },
  );
  ...
  if (row.hotel_id !== hotel.id) {
    throw new Error("RUNTIME_CANARY_ROUTE_HOTEL_MISMATCH");
  }
  return mapRoute(row);
}
...
async function loadJwks(issuer: string) {
  ...
  const jwksUrl = new URL("/.well-known/jwks", `${issuer.replace(/\/$/, "")}/`);
  ...
  if (!response.ok) {
    throw new Error(`RUNTIME_FORWARD_OIDC_JWKS_FAILED:${response.status}`);
  }
...
async function verifyVercelOidcToken(token: string) {
  ...
  const allowedIssuer =
    ...
    (!!owner && issuer === `https://oidc.vercel.com/${owner}`);
  if (!allowedIssuer) throw new Error("RUNTIME_FORWARD_OIDC_ISSUER_INVALID");

  if (
    payload.owner_id !== EXPECTED_VERCEL_TEAM_ID ||
    payload.project_id !== EXPECTED_VERCEL_PROJECT_ID
  ) {
  ...
  if (nbf === null || exp === null || iat === null) {
    throw new Error("RUNTIME_FORWARD_OIDC_TIME_CLAIMS_INVALID");
  }
  if (
  ...
  const jwk = keys.find(
    (candidate) =>
      candidate.kid === header.kid && (!candidate.alg || candidate.alg === "RS256"),
  );
...
function positiveIntegerHeader(req: NextRequest, name: string) {
  const value = Number(req.headers.get(name) || "0");
  if (!Number.isSafeInteger(value) || value < 1) {
    throw new Error("RUNTIME_FORWARD_EVIDENCE_ID_INVALID");
  }
  return value;
}
...
async function acceptForwardedRequest(input: {
  ...
  if (!input.hotel.is_sandbox) {
    throw new Error("RUNTIME_FORWARD_PRODUCTION_FORBIDDEN");
  }

  const authorization = input.req.headers.get("authorization") || "";
  if (!authorization.startsWith("Bearer ")) {
    throw new Error("RUNTIME_FORWARD_OIDC_REQUIRED");
  }
  ...
  const targetGeneration = positiveIntegerHeader(
    input.req,
    "x-stayhub-runtime-target-generation",
  );
  const verificationEvidenceId = positiveIntegerHeader(
    input.req,
    "x-stayhub-runtime-verification-evidence-id",
  );
  const trafficLeaseEvidenceId = positiveIntegerHeader(
    input.req,
    "x-stayhub-runtime-traffic-lease-evidence-id",
  );
  const computeRef = input.req.headers.get("x-stayhub-runtime-compute-ref") || "";
  const computeOrigin = String(
    input.req.headers.get("x-stayhub-runtime-compute-origin") || "",
  ).toLowerCase();
  const routeValidUntil = Date.parse(
    input.req.headers.get("x-stayhub-runtime-route-valid-until") || "",
  );

  if (forwardedHotelId !== input.hotel.id) {
    throw new Error("RUNTIME_FORWARD_HOTEL_MISMATCH");
  }
  if (!/^[a-z0-9][a-z0-9_-]{0,62}$/.test(targetKey)) {
  ...
  if (parseAllowedComputeOrigin(computeRef) !== computeOrigin) {
    throw new Error("RUNTIME_FORWARD_COMPUTE_REF_MISMATCH");
  }

  const { data, error } = await supabaseAdmin.rpc(
    "validate_guest_sandbox_canary_forward_v1",
    {
      p_hotel_id: input.hotel.id,
      p_target_key: targetKey,
      p_target_generation: targetGeneration,
      p_verification_evidence_id: verificationEvidenceId,
      p_traffic_lease_evidence_id: trafficLeaseEvidenceId,
      p_compute_ref: computeRef,
    },
  );
  if (error) {
    throw new Error(`RUNTIME_FORWARD_AUTHORITY_RECHECK_FAILED:${error.message}`);
  }
  if (data !== true) {
    throw new Error("RUNTIME_FORWARD_AUTHORITY_REVOKED");
  }
...
export async function maybeForwardSandboxGuestRequest(input: {
  ...
  const acceptedForward = await acceptForwardedRequest({
    req: input.req,
    hotel: input.hotel,
  });
  ...
  if (route.provider !== "stayhub") {
    throw new Error("RUNTIME_CANARY_PROVIDER_UNSUPPORTED");
  }
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "x-stayhub-runtime-verification-evidence-id": String(
        route.verificationEvidenceId,
      ),
      "x-stayhub-runtime-traffic-lease-evidence-id": String(
        route.trafficLeaseEvidenceId,
      ),
      "x-stayhub-runtime-compute-ref": route.computeRef,
      "x-stayhub-runtime-compute-origin": targetOrigin,
      "x-stayhub-runtime-route-valid-until": route.routeValidUntil,
      "x-stayhub-runtime-source-vercel-id":
        input.req.headers.get("x-vercel-id") || "",
    },
...
export function runtimeCanaryRoutingErrorResponse(error: unknown) {
  const message =
    error instanceof Error
      ? error.message
      : String(error || "RUNTIME_CANARY_ROUTING_FAILED");
