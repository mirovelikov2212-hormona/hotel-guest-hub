...
const EXISTENCE_ONLY_BASES = new Set(["canonical_service_text_entity"]);
...
function domainCompleteness(domainInventory, profile) {
  ...
  const deterministicPresenceItems = matchableItems.filter((item) =>
    Boolean(item?.crawled) && EXISTENCE_ONLY_BASES.has(key(item?.basis)));
  ...
  const missingDetailItems = expectedItems
    ...
    .filter((item) => {
      const basis = key(item?.basis);
      if (domain === "services" && EXISTENCE_ONLY_BASES.has(basis)) return false;
