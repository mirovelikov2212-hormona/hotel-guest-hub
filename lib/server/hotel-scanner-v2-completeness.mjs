...
export function buildHotelCompletenessV2(input = {}) {
  ...
  const onboardingDomains = domains.filter((domain) => domain.content?.status === "ONBOARDING_REQUIRED");
  ...
  if (inventoryConflictCount) {
    status = "INCOMPLETE";
    blockingReasons.push("inventory_expectation_conflict");
  }
  if (pendingDocuments.length) {
    status = "INCOMPLETE";
    blockingReasons.push("documents_pending_ingestion");
  }
  if (!inventoryIncompleteDomains.length && !inventoryConflictCount && !pendingDocuments.length && unresolvedConflicts > 0) {
  ...
  return {
    ...
    onboarding: {
      ...
      domains: onboardingDomains.map((domain) => domain.domain),
