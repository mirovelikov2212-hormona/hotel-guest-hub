...
const DOMAIN_FACT_RULES = Object.freeze({
  ...
  contacts: (fact) => key(fact?.category) === "contact" || ["phone", "email", "website", "social_profile"].includes(key(fact?.attribute)),
});
...
function matchingFactForItem(item, facts, profile, domain) {
  ...
  if (DETAIL_BASES.has(basis)) {
    ...
    return facts.find((fact) => sourceMatchesItem(fact, item) && Boolean(factEntityKey(fact)) && (!expectedName || factEntityKey(fact) === expectedName))
      || facts.find((fact) => sourceMatchesItem(fact, item) && Boolean(factEntityKey(fact)))
      || null;
  }
