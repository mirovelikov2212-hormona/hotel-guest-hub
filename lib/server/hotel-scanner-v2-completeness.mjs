...
const ENTITY_ATTRIBUTES = new Set([
  "room_type", "venue", "service", "treatment", "facility", "amenity",
  "experience", "activity", "attraction", "offer",
]);
const GENERIC_SUBJECTS = new Set(["hotel", "resort", "property", "accommodation", "gastronomy", "spa", "wellness", "services"]);
...
function matchingFactForItem(item, facts, profile, domain) {
  ...
  if (basis === "deterministic_landing_entity") {
...
function domainCompleteness(domainInventory, profile) {
  ...
  const matched = matchableItems
    .map((item) => ({ item, fact: matchingFactForItem(item, facts, profile, domain) }))
  ...
  return {
    ...
    reason: inventoryConflict
      ? "expected_inventory_conflict"
      : complete
        ? "expected_inventory_covered"
        : "expected_inventory_missing_evidence",
    expected,
    extracted,
    missingItems,
...
export function buildHotelCompletenessV2(input = {}) {
  ...
  const inventoryConflictCount = (Array.isArray(inventory.domains) ? inventory.domains : [])
    .filter((domain) => clean(domain?.expectationState, 40) === "CONFLICT").length;
  ...
  return {
    ...
    conflicts: {
      ...
      inventory: inventoryConflictCount,
    },
