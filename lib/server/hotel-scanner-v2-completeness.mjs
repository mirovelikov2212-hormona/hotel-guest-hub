...
const GROUP_SURFACE_BASES = new Set([
  "canonical_section_entity",
  "deterministic_semantic_block_entity",
]);
...
function substantiveFactsForItem(item, facts, domain, itemEntityKeys) {
  const entityHint = itemEntityKeys.get(item.id) || "";
  const aliases = detailEntityAliases(item, facts, entityHint || entityKey(item?.nameHint));
  return factsForItemContent(item, facts, domain, entityHint)
    .filter((fact) => !isIdentityOnlyFact(fact, item, aliases));
}

function groupSurfaceCoveredByDetailedSiblings(item, expectedItems, extractedIds, facts, domain, itemEntityKeys) {
  if (!GROUP_SURFACE_BASES.has(key(item?.basis))) return false;
  const primary = primarySourceUrl(item);
  if (!primary) return false;

  const siblingKeys = new Set();
  for (const sibling of expectedItems) {
    if (sibling?.id === item?.id || !extractedIds.has(sibling?.id)) continue;
    if (key(sibling?.basis) === "deterministic_explicit_count_slot") continue;
    if (!expectedUrls(sibling).has(primary)) continue;

    const siblingKey = entityKey(sibling?.nameHint);
    if (!siblingKey || sameEntityName(siblingKey, entityKey(item?.nameHint))) continue;
    if (substantiveFactsForItem(sibling, facts, domain, itemEntityKeys).length === 0) continue;
    siblingKeys.add(siblingKey);
    if (siblingKeys.size >= 2) return true;
  }
  return false;
}
...
function notDiscoveredDomain(domain) {
  const reason = "surface_not_discovered";
  const inventory = { status: "NOT_DISCOVERED", reason, expected: 0, extracted: 0, missingItems: [], extractedItemIds: [] };
  const content = { status: "NOT_DISCOVERED", reason, detailed: 0, totalEntities: 0, missingDetailItems: [] };
  return {
    domain, status: "NOT_DISCOVERED", reason, expected: 0, extracted: 0, missingItems: [], extractedItemIds: [],
    inventory, content, blocking: false,
  };
}
...
function domainCompleteness(domainInventory, profile) {
  ...
  if (expectationState === "UNKNOWN") {
    const inventory = { status: "ONBOARDING_REQUIRED", reason: "manual_onboarding_inventory_pending", expected: null, extracted: 0, missingItems: [], extractedItemIds: [] };
    const content = { status: "ONBOARDING_REQUIRED", reason: "manual_onboarding_inventory_pending", detailed: 0, totalEntities: 0, missingDetailItems: [] };
    return {
      domain, status: "ONBOARDING_REQUIRED", reason: "manual_onboarding_inventory_pending", expected: null, extracted: 0, missingItems: [], extractedItemIds: [],
      inventory, content, blocking: true,
    };
  }
  if (expectationState === "ABSENT") return notDiscoveredDomain(domain);
  ...
  const inventory = {
    status: inventoryComplete ? "COMPLETE" : "ONBOARDING_REQUIRED",
    reason: inventoryConflict ? "expected_inventory_conflict" : inventoryComplete ? "expected_inventory_covered" : "manual_onboarding_inventory_pending",
    expected,
    extracted,
    missingItems,
  ...
  const missingDetailItems = expectedItems
    ...
    .filter((item) => {
      ...
      if (groupSurfaceCoveredByDetailedSiblings(item, expectedItems, extractedIds, facts, domain, itemEntityKeys)) return false;
      return substantiveFactsForItem(item, facts, domain, itemEntityKeys).length === 0;
    })
    .map(itemSummary);
  ...
  const content = {
    status: contentComplete ? "COMPLETE" : "ONBOARDING_REQUIRED",
    reason: contentComplete ? "entity_details_covered" : "manual_onboarding_details_pending",
    detailed,
    totalEntities: extracted,
    missingDetailItems,
  };

  const ready = inventoryComplete;
  const reason = !inventoryComplete
    ? inventory.reason
    : contentComplete
      ? "inventory_and_content_complete"
      : "inventory_ready_onboarding_details_pending";
  return {
    domain,
    status: ready ? "COMPLETE" : "ONBOARDING_REQUIRED",
    reason,
    expected,
    extracted,
    missingItems,
...
export function buildHotelCompletenessV2(input = {}) {
  ...
  const allDocuments = Array.isArray(inventory.documents) ? inventory.documents : [];
  const pendingDocuments = allDocuments.filter((document) => !["ingested", "manual"].includes(key(document?.ingestionStatus)));
  const manualDocuments = allDocuments.filter((document) => key(document?.ingestionStatus) === "manual");
  const unresolvedConflicts = unresolvedConflictCount(input.conflicts);
  const approvalInventoryDomains = domains.filter((domain) => domain.inventory?.status === "ONBOARDING_REQUIRED");
  const onboardingDomains = domains.filter((domain) =>
    domain.status === "ONBOARDING_REQUIRED"
    || domain.status === "NOT_DISCOVERED"
    || domain.content?.status === "ONBOARDING_REQUIRED");
  ...
  // A completed Scanner run is ready for client preview/onboarding even when
  // some inventory needs human setup. Approval remains a stricter, separate gate.
  const status = "READY_FOR_ONBOARDING";
  const blockingReasons = [];
  if (approvalInventoryDomains.length) blockingReasons.push("domain_inventory_onboarding_required");
  if (inventoryConflictCount) blockingReasons.push("inventory_expectation_conflict");
  if (pendingDocuments.length) blockingReasons.push("documents_pending_ingestion");
  if (unresolvedConflicts > 0) blockingReasons.push("unresolved_cross_source_conflicts");

  const prerequisitesSatisfied = blockingReasons.length === 0;
  return {
    schemaVersion: "hotel-completeness-v2",
    status,
    domains,
    documents: {
      discovered: allDocuments.length,
      ingested: allDocuments.filter((document) => key(document?.ingestionStatus) === "ingested").length,
    ...
    onboarding: {
      domainCount: onboardingDomains.length,
      domains: [...new Set(onboardingDomains.map((domain) => domain.domain))],
      missingDetailCount: onboardingDomains.reduce((sum, domain) => sum + Number(domain.content?.missingDetailItems?.length || 0), 0),
    },
