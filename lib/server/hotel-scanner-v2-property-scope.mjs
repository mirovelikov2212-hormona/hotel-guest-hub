...
const OPERATIONAL_OBJECT_SLUG = /(?:^|-)(?:room|zimmer|oda|suite|restaurant|restoran|bar|spa|wellness|pool|havuz|hallenbad|aquapark|aqua-park|kids-club|kichi-kids-club|game-room|spielzimmer|meeting-room|meeting-hall|konferenzraum|toplanti-salonu|gym|fitness|fitnessstudio|beach|strand|plaj|amphitheatre|amphitheater|amfitiyatro|disco|disko|food-court|info-corner|strandclub)(?:-\d+)?$/iu;
const EDITORIAL_ARTICLE_SLUG_PREFIX = /^(?:news|blog|article|story|press|media|guide|travel-guide|reisefuhrer|ratgeber|haber|haberler|nachricht|actualites|novosti)-/iu;
const COMMERCIAL_SLUG = /(?:offer|package|promotion|campaign|discount|deal|angebot|paket|rabatt|teklif|kampanya|indirim|honeymoon|balayi|balayı|kids?-free|children-free|cocuk-ucretsiz|çocuk-ücretsiz)/iu;
...
export function isLikelyHotelEditorialUrlV2(rawUrl) {
  ...
  const operationalObjectSlug = OPERATIONAL_OBJECT_SLUG.test(slug);
  if (EDITORIAL_ARTICLE_SLUG_PREFIX.test(slug)) return true;
  if (COMMERCIAL_SLUG.test(slug)) return false;

  // Hotel object/detail slugs are normally short and noun-like. Long prose
  // slugs are a strong CMS article/SEO signal across languages. A long slug
  // is kept only when it structurally ends in a known hotel-object noun.
  if (tokens.length >= 10) return true;
  if (hasYear && tokens.length >= 5) return true;
  if (tokens.length >= 7 && !operationalObjectSlug) return true;
