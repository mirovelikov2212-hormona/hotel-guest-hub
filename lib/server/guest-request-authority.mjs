...
export function resolveGuestRequestAuthority(input) {
  ...
  const department = VALID_DEPARTMENTS.has(configuredDepartment)
    ...
    : null;
  const configuredAfterHoursDepartment = normalizeKey(
    def?.afterHoursDepartment,
  );
  const afterHoursDepartment = VALID_DEPARTMENTS.has(
    configuredAfterHoursDepartment,
  )
    ? configuredAfterHoursDepartment
    : null;
