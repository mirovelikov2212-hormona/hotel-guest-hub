const DEPARTMENT_KEY_PATTERN = /^[a-z][a-z0-9_-]{0,62}$/;
...
function normalizeDepartment(value) {
  ...
  if (!DEPARTMENT_KEY_PATTERN.test(normalized)) return null;
  if (normalized === "manager" || normalized === "none") return null;
  return normalized;
}
...
function normalizeStaffLabels(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const labels = {
    bg: normalizeOptionalText(value.bg),
    en: normalizeOptionalText(value.en),
    de: normalizeOptionalText(value.de),
  };
  return labels.bg || labels.en || labels.de ? labels : null;
}
...
function findRequestDef(requestDefs, rawType, sourceRequestDef) {
  ...
  const sourceId = String(sourceRequestDef ?? "").trim();
  const rawKey = normalizeKey(rawType);

  if (sourceId) {
    const byId = defs.find((def) => String(def?.id ?? "").trim() === sourceId);
    if (byId) return { def: byId, matchedBy: "source" };

    const byCanonicalRef = defs.find(
      (def) => String(def?.canonicalRef ?? "").trim() === sourceId,
    );
...
export function resolveGuestRequestAuthority(input) {
  ...
  const requestedSourceRequestDef = normalizeOptionalText(input?.sourceRequestDef);
  ...
  if (input?.strictConfiguredRequests === true && !lookup.def) {
    return {
      ok: false,
      code: "REQUEST_DEF_NOT_FOUND",
      message: "The requested hotel service is not available in the current configuration.",
  ...
  const department = normalizeDepartment(def?.targetDepartment);
  const afterHoursDepartment = normalizeDepartment(def?.afterHoursDepartment);
  ...
  const staffLabels = normalizeStaffLabels(def?.staffLabel);

  return {
    ok: true,
    requestType: authoritativeRequestType || rawType,
    department,
    ...(afterHoursDepartment ? { afterHoursDepartment } : {}),
    notifyDepartments,
    requiresBilling,
