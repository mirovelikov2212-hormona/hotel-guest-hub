const MAX_BLOCK_LINK_SCAN_BYTES = 1_500_000;
const MAX_BLOCK_LINK_ITEMS = 256;
...
function blockLinkItems(html) {
  const result = [];
  for (const match of String(html || "").matchAll(/<a\b[^>]*\bhref\s*=\s*["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    const href = clean(match[1], 2_048);
    const text = clean(match[2], 500);
    if (!href) continue;
    const key = href + "|" + text.toLocaleLowerCase("en-US");
    if (!result.some((item) => item.href + "|" + item.text.toLocaleLowerCase("en-US") === key)) {
      result.push({ href, text });
    }
    if (result.length >= MAX_BLOCK_LINK_ITEMS) break;
  }
  return result;
}

function blockLinks(html) {
  return [...new Set(blockLinkItems(html).map((item) => item.href))].slice(0, 12);
}
...
function contentBlocks(source) {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    while (hierarchy.length && hierarchy[hierarchy.length - 1].level >= level) hierarchy.pop();
    const sectionPath = hierarchy.map((item) => item.heading).slice(-5);
    ...
    // Keep semantic text extraction tightly bounded, but allow a larger
    // still-bounded window for anchors. Long hotel landing sections can place
    // late room/offer/service cards after substantial descriptive markup.
    const blockHtml = html.slice(start, end);
    const textHtml = blockHtml.slice(0, 24_000);
    const linkHtml = blockHtml.slice(0, MAX_BLOCK_LINK_SCAN_BYTES);
    const text = clean(textHtml, 4_000);
    blocks.push({
      level,
      heading,
      text,
      links: blockLinks(linkHtml),
      linkItems: blockLinkItems(linkHtml),
      sectionPath,
    });
    hierarchy.push({ level, heading });
  }
  return uniqueBy(blocks, (item) => `${item.level}|${item.sectionPath.join("/").toLocaleLowerCase("en-US")}|${item.heading.toLocaleLowerCase("en-US")}|${item.text.slice(0, 120).toLocaleLowerCase("en-US")}`);
}
