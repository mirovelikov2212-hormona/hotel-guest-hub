...
function isAccommodationContainerName(value) {
  const name = clean(value, 240);
  return /^(?:rooms?\s*(?:&|and)\s*suites?)(?:\s+(?:in|at)\s+.+)?$|^zimmer\s*(?:&|und)\s*suiten(?:\s+im\s+.+)?$/iu.test(name);
}
  ...
  for (const resource of resources) {
    ...
    for (const source of sources) {
      ...
      for (const candidate of source.candidates) {
        ...
        if (!name || !key || isAccommodationContainerName(name) || (entityType && entityType !== "room_type") || seen.has(key)) continue;
        seen.add(key);
