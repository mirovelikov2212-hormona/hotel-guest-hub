...
function familyAdaptiveState(family, sourceFamilyUrls, covered, unavailable) {
  ...
  const expandedMemberIndexes = members
    .map((member, index) => sourceFamilyUrls.has(hotelScannerV3LogicalUrlKey(member?.url)) ? index : -1)
    .filter((index) => index >= 0);

  const expandAll = expandedMemberIndexes.length > 0;
  const sample = sampleIndexes(members.length, family.evidenceSource);
  const requiredIndexes = expandAll
  ...
  const inferredLeafMembers = expandAll
    ? 0
    : members.filter((_, index) => !requiredIndexes.includes(index)).length;
...
function buildDiscoveryCandidates(evidence, covered, unavailable, managedFamilyKeys) {
  ...
  for (const page of evidence.pages || []) {
    if (!entryKeys.has(hotelScannerV3LogicalUrlKey(page?.url))) continue;
...
export function buildHotelScannerAdaptivePlanV3(evidence = {}, options = {}) {
  ...
  const closedFamilies = families.states.filter((state) =>
    state.status === "CLOSED_EXPANDED" || state.status === "CLOSED_INFERRED_LEAF");
