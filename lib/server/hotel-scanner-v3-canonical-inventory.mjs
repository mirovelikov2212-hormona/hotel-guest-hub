...
const DOMAIN_MANUAL_ONLY = new Set(["experiences", "events"]);

function domainConflictEntityIds(domain, conflicts) {
  return conflicts.filter((entity) =>
    (entity?.provenance || []).some((claim) => claim?.domain === domain))
    .map((entity) => entity.id)
    .sort();
}

function domainAuthoritySummaries(entities, structuralInventory, ontology, evidence) {
  const stateMap = structuralFamilyStateMap(evidence);
  const families = structuralInventory?.families || [];
  const ontologyByFamilyId = new Map((ontology?.families || []).map((family) => [family.familyId, family]));
  const conflicts = entities.filter((entity) => entity.status === "ONTOLOGY_CONFLICT");

  return HOTEL_SCANNER_V3_OPERATIONAL_DOMAINS.map((domain) => {
    const values = entities.filter((entity) => entity.domain === domain && entity.status === "CLASSIFIED");
    const familyIds = (ontology?.families || [])
      .filter((family) => family.domain === domain)
      .map((family) => family.familyId)
      .filter(Boolean);
    const uncertifiedFamilyIds = families
      .filter((family) => familyIds.includes(family.id) && !familyCertifiedForAuthority(family.id, stateMap))
      .map((family) => family.id)
      .sort();
    const conflictEntityIds = domainConflictEntityIds(domain, conflicts);
    const structuralEntityCount = values.filter((entity) => entity.memberMode !== "capability_assertion").length;
    const assertionEntityCount = values.length - structuralEntityCount;

    let authorityStatus = "NOT_DISCOVERED";
    const blockers = [];
    if (DOMAIN_MANUAL_ONLY.has(domain)) {
      authorityStatus = "MANUAL";
      blockers.push("manual_only_domain");
    } else if (conflictEntityIds.length) {
      authorityStatus = "CONFLICT";
      blockers.push("ontology_conflict");
    } else if (uncertifiedFamilyIds.length) {
      authorityStatus = "PARTIAL";
      blockers.push("structural_family_open");
    } else if (domain === "services" && values.length && !structuralEntityCount) {
      // Deterministic capability assertions are useful preview evidence but are
      // not proof that the service inventory is exhaustive.
      authorityStatus = "PARTIAL";
      blockers.push("assertions_not_exhaustive");
    } else if (values.length) {
      authorityStatus = "READY";
    } else if (familyIds.length) {
      authorityStatus = "PARTIAL";
      blockers.push("family_without_terminal_entities");
    }
    ...
    return {
      ...
      status: values.length ? "DISCOVERED" : "NOT_DISCOVERED",
      authorityStatus,
      familyIds: unique(familyIds).sort(),
      uncertifiedFamilyIds,
      conflictEntityIds,
      structuralEntityCount,
      assertionEntityCount,
      blockers,
    };
  });
}

function authorityDomainReady(domain, authorityStatusFallback = "") {
  const status = clean(domain?.authorityStatus, 80);
  if (status) return status === "READY";
  return authorityStatusFallback === "READY" && clean(domain?.status, 80) === "DISCOVERED";
}

export function hasReadyHotelInventoryAuthorityV3(authority = {}) {
  return (authority?.domains || []).some((domain) =>
    authorityDomainReady(domain, clean(authority?.status, 80)));
}
...
export function buildHotelInventorySnapshotV3(evidence = {}, options = {}) {
  ...
  const domains = domainAuthoritySummaries(entities, structuralInventory, ontology, evidence);
  ...
  const uncertifiedFamilies = structuralStateMap.size
    ? (structuralInventory?.families || []).filter((family) => {
        if (familyCertifiedForAuthority(family.id, structuralStateMap)) return false;
        const domain = ontologyByFamilyId.get(family.id)?.domain || "UNKNOWN";
        return !DOMAIN_MANUAL_ONLY.has(domain);
      })
    : [];
...
export function compareHotelInventorySnapshotsV3(previous = {}, next = {}) {
  const previousStatus = clean(previous?.status, 80);
  const nextStatus = clean(next?.status, 80);
  const previousReadyDomains = new Set(
    (previous?.domains || [])
      .filter((domain) => authorityDomainReady(domain, previousStatus))
      .map((domain) => clean(domain?.domain, 80))
      .filter(Boolean),
  );
  const nextReadyDomains = new Set(
    (next?.domains || [])
      .filter((domain) => authorityDomainReady(domain, nextStatus))
      .map((domain) => clean(domain?.domain, 80))
      .filter(Boolean),
  );

  const previousById = new Map((previous?.entities || [])
    .filter((entity) => previousReadyDomains.has(entity?.domain))
    .map((entity) => [entity.id, entity]));
  const nextById = new Map((next?.entities || [])
    .filter((entity) => previousReadyDomains.has(entity?.domain))
    .map((entity) => [entity.id, entity]));
  ...
  const authorityLostDomains = [...previousReadyDomains]
    .filter((domain) => !nextReadyDomains.has(domain))
    .sort();

  return {
    schemaVersion: "hotel-scanner-v3-inventory-delta-1",
    changed: Boolean(
      addedEntityIds.length
      || removedEntityIds.length
      || domainChangedEntityIds.length
      || authorityLostDomains.length
    ),
    previousSnapshotId: clean(previous?.snapshotId, 200),
    nextSnapshotId: clean(next?.snapshotId, 200),
    comparedDomains: [...previousReadyDomains].sort(),
    authorityLostDomains,
    addedEntityIds,
    removedEntityIds,
...
export function projectHotelInventoryAuthorityV3(snapshot = {}) {
  ...
  return {
    ...
    domains: (snapshot?.domains || []).map((domain) => ({
      ...
      authorityStatus: clean(domain?.authorityStatus, 80),
      familyIds: unique(domain?.familyIds || []).sort(),
      uncertifiedFamilyIds: unique(domain?.uncertifiedFamilyIds || []).sort(),
      conflictEntityIds: unique(domain?.conflictEntityIds || []).sort(),
      blockers: unique(domain?.blockers || []).sort(),
      structuralEntityCount: Number(domain?.structuralEntityCount || 0),
      assertionEntityCount: Number(domain?.assertionEntityCount || 0),
    })),
    entities,
  };
}
...
export function applyHotelInventoryAuthorityV3(legacyInventory = {}, authority = {}) {
  ...
  for (const domain of HOTEL_SCANNER_V3_OPERATIONAL_DOMAINS) {
    const authorityDomain = (authority?.domains || []).find((entry) => entry?.domain === domain);
    if (!authorityDomainReady(authorityDomain, clean(authority?.status, 80))) continue;
  ...
  return {
    ...
    v3Authority: {
      ...
      readyDomains: (authority?.domains || [])
        .filter((domain) => authorityDomainReady(domain, clean(authority?.status, 80)))
        .map((domain) => clean(domain?.domain, 80))
        .filter(Boolean)
        .sort(),
    },
...
export function mergeHotelInventoryAuthoritiesV3(locked = null, observed = null) {
  if (!locked) return observed;
  if (!observed) return locked;

  const lockedStatus = clean(locked?.status, 80);
  const observedStatus = clean(observed?.status, 80);
  const selectedDomains = [];
  const lockedReadyDomains = new Set();

  for (const domainName of HOTEL_SCANNER_V3_OPERATIONAL_DOMAINS) {
    const lockedDomain = (locked?.domains || []).find((domain) => domain?.domain === domainName);
    const observedDomain = (observed?.domains || []).find((domain) => domain?.domain === domainName);
    if (authorityDomainReady(lockedDomain, lockedStatus)) {
      selectedDomains.push({ ...lockedDomain });
      lockedReadyDomains.add(domainName);
    } else if (observedDomain) {
      selectedDomains.push({ ...observedDomain });
    }
  }

  const lockedEntities = (locked?.entities || []).filter((entity) => lockedReadyDomains.has(entity?.domain));
  const observedEntities = (observed?.entities || []).filter((entity) => !lockedReadyDomains.has(entity?.domain));
  const entityMap = new Map();
  for (const entity of [...lockedEntities, ...observedEntities]) {
  ...
  return {
    ...
    lockedSnapshotId: clean(locked?.snapshotId, 200),
    observedSnapshotId: clean(observed?.snapshotId, 200),
  };
}
