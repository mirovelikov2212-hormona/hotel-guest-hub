...
const QUESTION_HEADING = /(?:\?|^(?:does|do|is|are|can|could|may|what|where|when|how|which|who)\b|^(?:има ли|предлага ли|може ли|как|къде|кога|кой|коя|какво)\b|^(?:gibt es|bietet|kann|wie|wo|wann|welche?r?)\b|^(?:există|oferă|se poate|cum|unde|când|care)\b|^(?:var mı|var mi|sunuyor mu|sunuluyor mu|mümkün mü|mümkün mu|nasıl|nasil|nerede|ne zaman|hangi)\b)/iu;
const AWARD_RECOGNITION = /(?:\bawards?\b|\baward-winning\b|\bcertificate\b|\bcertification\b|\brecognition\b|\btripadvisor\b|\bholidaycheck\b|\btravelife\b|\bödül(?:ler|ü)?\b|\bsertifika\b|\bsertifikalar\b|\bauszeichnung(?:en)?\b|\bpreise?\b|\bnagrod(?:a|y)\b|\bнагради?\b|\bсертификат(?:и)?\b)/iu;

const WELLNESS_CORE = /(?:\bspa\b|\bwellness\b|\bhamam\b|\bhammam\b|\bsauna\b|\bsteam(?: room| bath)?\b|\bmassage\b|\bmasaj\b|\bterapi\b|\btreatment\b|\btherapy\b|\britual\b|\bmedical spa\b|\bbalneo\b|\bуелнес\b|\bспа\b|\bмасаж\b|\bсауна\b|\bтерап/u;
...
export function isFaqQuestionHeadingV2(value) {
  return QUESTION_HEADING.test(clean(value, 300));
}

export function isAwardRecognitionHeadingV2(value) {
  return AWARD_RECOGNITION.test(clean(value, 500));
}

export function isWellnessContextV2(value) {
  return WELLNESS_CORE.test(clean(value, 2000));
}

export function classifyCommonHotelObjectV2(label, context = "", primaryDomain = "") {
  const name = clean(label, 300);
  const text = `${name} ${clean(context, 2500)}`;
  if (!name || isFaqQuestionHeadingV2(name) || isAwardRecognitionHeadingV2(name)) return null;
