...
const PROMOTIONAL_HEADING = /(?:\b(?:special\s+offers?|offers?|packages?|promotions?|campaigns?|discounts?|deals?)\b|\b(?:sonderangebote?|angebote|pakete?|rabatt)\b|\b(?:teklifler?|kampanyalar?|paketler?|indirim)\b|\b(?:honeymoon|balay[ıi])\b|(?:kids?|children|child|kinder|cocuk|çocuk).{0,40}(?:free|kostenlos|ucretsiz|ücretsiz)|(?:guests?|famil(?:y|ies)|misafir(?:ler|lere)?|aile(?:ler|lere)?).{0,50}(?:special|exclusive|ozel|özel)|(?:special|exclusive|ozel|özel).{0,40}(?:guests?|famil(?:y|ies)|children|kids?|misafir(?:ler|lere)?|aile(?:ler|lere)?))/iu;
...
const POOL = /(?:\bpool(?:s)?\b|\bswimming pool\b|\bhavuz(?:lar)?\b|\byuzme havuzu\b|басейн(?:и)?|\bschwimmbad\b|\bhallenbad\b|\bpiscin(?:a|e|a)\b)/iu;
...
const GAMES = /(?:\bgame\s*(?:hall|room|area|zone)\b|\bgames\s*room\b|\barcade\b|\boyun\s*(?:salonu|odasi|alani)\b|\bspielzimmer\b|\bspielraum\b|игрална\s*(?:зала|стая|зона))/iu;
const MEETING_FACILITY = /(?:\bmeeting\s*(?:room|hall)\b|\bconference\s*(?:room|hall|centre|center)\b|\bconvention\s*(?:room|hall|centre|center)\b|\btoplanti\s*(?:salonu|odasi)\b|\btagungsraum\b|\bkonferenzraum\b|конферентна\s+зала|зала\s+за\s+срещи)/iu;
const SPORTS = /(?:\bsports?\b|\bfitness\b|\bfitnessstudio\b|\bgym\b|\btennis\b|\bvolleyball\b|\bbeach volleyball\b|\bfootball\b|\bsoccer\b|\bbasketball\b|\bmultifunctional playground\b|\bspor\b|\bspor salonu\b|\btenis\b|\bvoleybol\b|\bfutbol\b|\bbasketbol\b|фитнес|спорт|тенис|волейбол|футбол)/iu;
const ENTERTAINMENT = /(?:\banimation\b|\bentertainment\b|\bshows?\b|\bpart(?:y|ies)\b|\bmini\s*disco\b|\bdisco\b|\bdisko\b|\bnight\s*club\b|\bnightclub\b|\bamphitheat(?:re|er)\b|\bamfitiyatro\b|\banimasyon\b|\beglence\b|\bgosteri\b|анимация|шоу|забавления|амфитеатър)/iu;
...
const RESTAURANT = /(?:\brestaurant\b|\bhauptrestaurant\b|\ba['’]?la\s+carte\s+restaurant\b|\brestoran\b|\blokanta\b|\bristorante\b|\brestaurante?\b|\brestaurace\b|ресторант)/iu;
const BAR = /(?:\bbar\b|\bcafe\b|\bkafe\b|\bbistro\b|\bpub\b|\blounge\b|\bsnack bar\b|кафе|бар)/iu;
const OPERATIONAL_SERVICE = /(?:\bparking\b|\bparkplatz\b|\bparkgarage\b|\btiefgarage\b|\bgarage\b|\botopark\b|\btransfer\b|\bairport transfer\b|\bhavalimani transfer\b|\blaundry\b|\bcamasirhane\b|\breception\b|\bresepsiyon\b|\bwi-?fi\b|\bwlan\b|\bwireless\b|\bshop\b|\bstore\b|\bmagaza\b|\bexchange\b|\bcurrency exchange\b|\bdoviz\b|\bhairdresser\b|\bhair salon\b|\bbarbershop\b|\bkuafor\b|\bbeauty salo+n\b|\bguzellik salonu\b|\bmedical service\b|\bdoctor\b|\bdoktor\b|\binfo(?:rmation)?\s*(?:corner|desk|point)\b|\bconcierge(?: service)?\b|\bguest relations?\b|\bluggage storage\b|\bbaggage storage\b|\broom service\b|\bwake[- ]?up service\b|\bbabysitting\b|\bkinderbetreuung\b|\bchildcare\b|\bcar rental\b|\brent a car\b|\bski rental\b|\bskiverleih\b|\bski depot\b|\bskidepot\b|\be-?ladestationen?\b|\bev charging\b|\bcharging stations?\b|\bbusiness cent(?:er|re)\b|\bbell desk\b|паркинг|трансфер|пералн|рецепция|магазин|обменно бюро|фризьор)/iu;
...
export function isHotelPromotionHeadingV2(value) {
  return PROMOTIONAL_HEADING.test(searchable(value, 700));
}
...
export function classifyCommonHotelObjectV2(label, context = "", primaryDomain = "") {
  ...
  // Facility nouns containing the word "room" must be resolved before the
  // accommodation rule. A Game Room or Meeting Room is not a guest room type.
  if (GAMES.test(name)) return { domain: "experiences", entityType: "games_facility" };
  if (MEETING_FACILITY.test(name)) return { domain: "services", entityType: "meeting_facility" };
...
const SERVICE_TEXT_PATTERNS = Object.freeze([
  { name: "24/7 Reception", pattern: /(?:24\s*\/\s*7|24\s*hours?|24\s*stunden|round\s+the\s+clock)[^\n.]{0,40}(?:reception(?:\s+desk)?|resepsiyon|rezeption|рецепц(?:ия|ията))|(?:reception(?:\s+desk)?|resepsiyon|rezeption|рецепц(?:ия|ията))[^\n.]{0,40}(?:24\s*\/\s*7|24\s*hours?|24\s*stunden|round\s+the\s+clock)/iu },
  { name: "Reception", pattern: /(?:reception(?:\s+desk)?|resepsiyon|rezeption|рецепц(?:ия|ията))/iu },
  { name: "Concierge", pattern: /(?:\bconcierge(?:\s+service)?\b|\bguest relations?\b)/iu },
  { name: "Laundry & Ironing", pattern: /(?:washing\s*&?\s*ironing|laundry|camasirhane|w[aä]scheservice|b[uü]gelservice|пералн|гладен)/iu },
  ...
  { name: "Wi-Fi", pattern: /(?:\bwi[ -]?fi\b|\bwlan\b|\bwireless\s+(?:internet|connection)\b)/iu },
  { name: "Parking", pattern: /(?:\bparking\b|\bparkplatz\b|\bparkgarage\b|\btiefgarage\b|\botopark\b|паркинг)/iu },
  { name: "EV Charging", pattern: /(?:\be-?ladestationen?\b|\bev\s+charging\b|\bcharging\s+stations?\b)/iu },
  { name: "Ski Rental", pattern: /(?:\bski\s+rental\b|\bskiverleih\b|\bski\s*&\s*snowboard\s+rental\b)/iu },
  { name: "Ski Depot", pattern: /(?:\bski\s+depot\b|\bskidepot\b)/iu },
  { name: "Childcare", pattern: /(?:\bchildcare\b|\bbabysitting\b|\bkinderbetreuung\b)/iu },
  { name: "Airport Transfer", pattern: /(?:airport\s+(?:shuttle|transfer)|havalimani\s+transfer|flughafen\s*transfer|летищен\s+трансфер)/iu },
]);
...
export function extractOperationalServiceLabelsV2(value) {
  ...
  if (result.some((item) => item.name === "24/7 Reception")) {
    return result.filter((item) => item.name !== "Reception");
  }
  return result;
}
