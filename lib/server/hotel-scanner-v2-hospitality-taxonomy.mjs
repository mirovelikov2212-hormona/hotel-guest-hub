...
export function classifyCommonHotelObjectV2(label, context = "", primaryDomain = "") {
  ...
  if (BEACH.test(name)) {
    ...
    const facilityLike = /^(?:private\s+|sandy\s+|hotel\s+|resort\s+)?beach(?:\s+area)?$|^(?:plaj|strand|плаж)(?:\s+area|\s+alan[ıi])?$/iu.test(name);
    if (brandLike && !facilityLike) return null;
    return { domain: "experiences", entityType: "beach" };
  }
