...
function domainPathDepth(rawUrl, rule) {
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    if (rule.path.test(`/${segment}/`)) return Math.max(0, segments.length - index - 1);

    // Hotel sites often compose navigation slugs such as "wohnen-angebote",
    // "hotel-services" or "spa-treatments". Treat delimiter-separated tokens
    // as structural path vocabulary without adding hotel-specific routes.
    const tokens = segment.split(/[-_.]+/u).filter(Boolean);
    for (const token of tokens) {
      if (token.length < 3) continue;
      if (rule.path.test(`/${token}/`)) return Math.max(0, segments.length - index - 1);
    }
  }
  return -1;
}
...
export function classifyHotelScannerPageV2(page = {}) {
  ...
  const pathLanding = DOMAIN_RULES
    .map((rule) => ({ rule, depth: domainPathDepth(url, rule) }))
