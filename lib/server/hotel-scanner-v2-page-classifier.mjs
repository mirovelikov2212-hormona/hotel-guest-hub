...
function isLocalizedHomePath(rawUrl) {
  const segments = pathSegments(rawUrl);
  if (!segments.length) return true;
  if (segments.length === 1 && LANGUAGE_SEGMENT.test(segments[0])) return true;
  if (segments.length === 2 && LANGUAGE_SEGMENT.test(segments[1])) return true;
  return false;
}
...
function strongObjectClusterDomain(page) {
  const counts = new Map();
  const headings = (Array.isArray(page?.headings) ? page.headings : []).slice(0, 12);
  for (const heading of headings) {
    const value = clean(heading?.text);
    if (!value) continue;
    const object = classifyCommonHotelObjectV2(value, value, "");
    if (!object?.domain) continue;
    counts.set(object.domain, (counts.get(object.domain) || 0) + 1);
  }
...
export function classifyHotelScannerPageV2(page = {}) {
  ...
  if (CORPORATE_GOVERNANCE_SURFACE.test(path) || CORPORATE_GOVERNANCE_SURFACE.test(clean(page?.title))) {
    return { primaryType: "other", types: ["other"], confidence: 1, signals: ["corporate_governance_surface"] };
  }
  ...
  if (isLocalizedHomePath(url)) {
    return { primaryType: "other", types: ["other"], confidence: 0.99, signals: ["localized_home_surface"] };
  }
  ...
  const clusterDomain = strongObjectClusterDomain(page);
  const candidates = DOMAIN_RULES.map((rule) => {
    ...
    const semanticAuthorityRequired = ["services", "offers"].includes(rule.key);
    const semanticMatch = rawSemanticMatch
      && (!semanticAuthorityRequired || pathMatch || titleSemanticMatch || clusterDomain === rule.key);
    let score = 0;
