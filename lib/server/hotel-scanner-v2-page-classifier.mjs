...
const CORPORATE_POLICY_SURFACE = /(?:policy|policies|politik|richtlinie|richtlinien|managementpolitik|nachhaltigkeitspolitik|lieferkettenpolitik|supply-chain-policy|data-security|stakeholder-rights|sustainability-policy|corporate-sustainability)/iu;
const LEGAL_SUPPORT_SURFACE = /(?:distance[- ]selling|sales[- ]contract|consumer[- ]contract|legal[- ]notice|clarification[- ]text|terms[- ]of[- ]sale|fernabsatzvertrag|kaufvertrag|klarstellungstext|mesafeli[- ]satis|mesafeli[- ]satış|sozlesme|sözleşme|yasal[- ]uyari|yasal[- ]uyarı)/iu;

const DOMAIN_RULES = Object.freeze([
  {
    key: "accommodation", landing: "accommodation", detail: "room_detail",
    path: /(?:^|\/)(?:rooms?|rooms?-suites?|accommodation|stay|suites?|apartments?|villas?|zimmer|zimmer-suiten|unterkunft|camere|camere-suite|cazare|pokoje|ubytovani|odalar?|suitler?|süitler?|konaklama|стаи?|настаняване|номера?|размещение)(?:\/|$)/iu,
  ...
  },
  {
    ...
    path: /(?:^|\/)(?:gastronomy|dining|restaurants?|bars?|food-drink|essen-trinken|gastronomie|mountain-cuisine|cuisine|kulinarik|gourmet|restaurante?|restoranlar?|barlar?|yeme-icme|yeme-içme|ресторанти?|барове?|гастрономия)(?:\/|$)/iu,
  ...
  },
  {
    ...
    path: /(?:^|\/)(?:spa|mountain-spa|wellness-spa|wellness|medical-spa|medical|balneo|balneology|healing|therap(?:y|ies)|treatments?|massages?|hamam|hammam|masaj|terapi|bakim|bakım|спа|уелнес|балнео|лечение|терапии?|процедури?|масажи?)(?:\/|$)/iu,
  ...
  },
  {
    ...
    path: /(?:^|\/)(?:services?|facilities|amenities|guest-services|inclusive-services?|included-services?|inclusive-benefits?|leistungen|inklusivleistungen|inklusive-leistungen|inklusiveleistungen|servicii|sluzby|hizmetler?|olanaklar|imkanlar|imkânlar|услуги|удобства)(?:\/|$)/iu,
    semantic: /(?:\bservices?\b|\bamenities\b|\bfacilities\b|\binclusive services?\b|\bincluded services?\b|\binclusive benefits?\b|\binklusivleistungen\b|\binklusive leistungen\b|\bconference hall\b|\bparking\b|\btransfer\b|\blaundry\b|\bhizmetler\b|\bolanaklar\b|\botopark\b|\bçamaşırhane\b|\bresepsiyon\b|\bуслуги\b|\bудобства\b|\bконферентна зала\b)/iu,
  },
  {
    key: "experiences", landing: "experiences", detail: "experience_detail",
    path: /(?:^|\/)(?:experiences?|activities|active|aktiv|things-to-do|discover|attractions?|hiking|hike|wandern|wanderung|bike|biking|cycling|ski|skiing|golf|yoga|tennis|fitness|aqua-?park|water-?park|pools?|sports?|kids?|children-area|animation|entertainment|beach|ausfluge|erlebnisse|experiente|zazitky|aktiviteler?|eglence|eğlence|aquapark|su-parki|su-parkı|havuzlar?|spor|cocuk-kulubu|çocuk-kulübü|plaj|преживявания|активности|забележителности|аквапарк|басейни?|спорт|анимация|плаж)(?:\/|$)/iu,
    semantic: /(?:\bexperiences?\b|\bactivities\b|\battractions?\b|\bhiking\b|\bhike\b|\bwander(?:n|ung|hotel)?\b|\bbik(?:e|ing)\b|\bcycling\b|\bski(?:ing)?\b|\bgolf\b|\byoga\b|\btennis\b|\bfitness\b|\baqua\s*park\b|\bwater\s*park\b|\bpools?\b|\bsports?\b|\bkids?\s*(?:club|area)\b|\banimation\b|\bentertainment\b|\bbeach\b|\baktiviteler\b|\baktivite\b|\beğlence\b|\beglence\b|\baquapark\b|\bsu park[ıi]\b|\bhavuz\b|\bspor\b|\bçocuk kulübü\b|\bcocuk kulubu\b|\bplaj\b|\berlebnisse\b|\bпреживявания\b|\bактивности\b|\bзабележителности\b|\bаквапарк\b|\bбасейн\b|\bспорт\b|\bанимация\b|\bплаж\b)/iu,
  },
  {
...
function domainPathMatch(rawUrl, rule) {
  const segments = semanticSegments(rawUrl);
  let compound = null;

  for (let index = 0; index < segments.length; index += 1) {
    const segment = segments[index];
    const depth = Math.max(0, segments.length - index - 1);

    // An exact path segment is structural route authority. Never allow a token
    // inside a later compound slug to outrank it.
    if (rule.path.test(`/${segment}/`)) {
      return { depth, index, kind: "exact" };
    }

    // Compound slugs are useful for a small set of navigation vocabularies
    // such as "wohnen-angebote" and "hotel-services". Do not split generic
    // hotel-object words such as "game-room": those are resolved semantically.
    if (!compound && ["offers", "services"].includes(rule.key)) {
      const tokens = segment.split(/[-_.]+/u).filter(Boolean);
      for (const token of tokens) {
        if (token.length < 3) continue;
        if (rule.path.test(`/${token}/`)) {
          compound = { depth, index, kind: "compound_token" };
          break;
        }
      }
    }
...
function domainPathDepth(rawUrl, rule) {
  return domainPathMatch(rawUrl, rule)?.depth ?? -1;
}
...
function strongestPathDomain(url) {
  const matches = DOMAIN_RULES
    .map((rule) => ({ rule, match: domainPathMatch(url, rule) }))
    .filter((entry) => entry.match)
    .sort((left, right) =>
      Number(right.match.kind === "exact") - Number(left.match.kind === "exact")
      || left.match.depth - right.match.depth
...
export function classifyHotelScannerPageV2(page = {}) {
  ...
  if (CORPORATE_POLICY_SURFACE.test(path) || CORPORATE_POLICY_SURFACE.test(clean(page?.title))) {
    return { primaryType: "policies", types: ["policies"], confidence: 0.99, signals: ["policy_surface"] };
  }
  ...
  const pathStandalone = standalone.find((entry) => entry.pathMatch);
  if (pathStandalone) {
    return { primaryType: pathStandalone.rule.type, types: [pathStandalone.rule.type], confidence: 1, signals: [`standalone_path:${pathStandalone.rule.type}`] };
  }

  const titleName = titleObjectName(page);
  const rawTitle = clean(page?.title);
  const pathDomain = strongestPathDomain(url);
  if ((LEGAL_SUPPORT_SURFACE.test(path) || LEGAL_SUPPORT_SURFACE.test(rawTitle)) && pathDomain !== "offers") {
    return { primaryType: "policies", types: ["policies"], confidence: 0.99, signals: ["legal_support_surface"] };
  }
  if (pathDomain !== "offers" && isHotelPromotionHeadingV2(rawTitle)) {
    return { primaryType: "offer_detail", types: ["offer_detail", "offers"], confidence: 0.96, signals: ["promotional_title"] };
  }

  // A recognized domain route with no segments beneath it is an explicit
  // hotel landing surface. Give that structural signal precedence over
  // long CMS/SEO titles, which may otherwise look editorial.
  const landingMatches = DOMAIN_RULES
    .map((rule) => ({ rule, match: domainPathMatch(url, rule) }))
    .filter((entry) => entry.match?.depth === 0);
  const exactLanding = landingMatches.find((entry) => entry.match.kind === "exact");
  const compoundLanding = semanticSegments(url).length === 1
    ? landingMatches.find((entry) => entry.match.kind === "compound_token")
    : null;
  const pathLanding = exactLanding || compoundLanding;
  if (pathLanding) {
    return {
      primaryType: pathLanding.rule.landing,
      types: [pathLanding.rule.landing],
      confidence: 0.99,
      signals: [`landing_path_authority:${pathLanding.rule.key}`],
    };
  }
  ...
  const objectOverrideAllowed = !isTaxonomyArchivePath(url)
    ...
    && !(pathDomain && pathDomain !== "accommodation" && titleObject.domain === "accommodation");
  ...
  const titleWords = rawTitle.split(/\s+/u).filter(Boolean).length;
  if (!pathDomain && !titleObject && titleWords >= 8) {
    return { primaryType: "other", types: ["other"], confidence: 0.92, signals: ["editorial_title_surface"] };
  }

  const semanticStandalone = standalone.find((entry) => entry.semanticMatch);
  if (semanticStandalone) {
    return { primaryType: semanticStandalone.rule.type, types: [semanticStandalone.rule.type], confidence: 0.9, signals: [`standalone_title:${semanticStandalone.rule.type}`] };
  }

  const candidates = DOMAIN_RULES.map((rule) => {
    const pathEvidence = domainPathMatch(url, rule);
    const depth = pathEvidence?.depth ?? -1;
    const pathMatch = Boolean(pathEvidence);
    ...
    const semanticMatch = rawSemanticMatch && (pathMatch || !["offers", "services"].includes(rule.key) || titleSemanticMatch);
    let score = 0;
    if (pathEvidence?.kind === "exact") {
      // /services/... is a generic container route and intentionally yields
      // to strong object semantics (restaurant, spa, kids, etc.).
      score += rule.key === "services" && depth > 0 ? 4 : 14;
    } else if (pathEvidence?.kind === "compound_token") score += 4;
    if (semanticMatch) score += 7;
    return { rule, depth, pathMatch, pathKind: pathEvidence?.kind || "", semanticMatch, score };
  }).filter((entry) => entry.score > 0)
    .sort((left, right) =>
      right.score - left.score
      || Number(right.pathKind === "exact") - Number(left.pathKind === "exact")
