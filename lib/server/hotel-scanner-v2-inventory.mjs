...
function inventoryHintsForDomain(resource, domain) {
  ...
  return hints
    .filter((hint) => hint?.domain === domain && Number(hint?.expectedCount || 0) > 0)
    .map((hint) => normalizedInventoryHint(domain, hint))
    .filter((hint) => Number(hint?.expectedCount || 0) > 0);
}
...
const GENERIC_OFFER_ENTITY = /^(?:offers?|special offers?|packages?|promotions?|angebote|sonderangebote|teklifler?|kampanyalar?|paketler?|firsatlar?|fırsatlar?|oferte|promoții|promotii|nabidky|nabídky|оферти|пакети|промоции)$/iu;

function normalizedInventoryHint(domain, hint) {
  if (domain !== "offers") return hint;
  const originalCandidates = Array.isArray(hint?.candidates) ? hint.candidates : [];
  const candidates = originalCandidates.filter((candidate) => !GENERIC_OFFER_ENTITY.test(clean(candidate?.name, 200)));
  if (candidates.length === originalCandidates.length) return hint;

  const originalExpected = Number(hint?.expectedCount || 0);
  const originalIdentified = Number(hint?.identifiedCount || originalCandidates.length || 0);
  const expectedCount = originalExpected === originalIdentified && candidates.length
    ? candidates.length
    : originalExpected;
  const explicitCount = Number(hint?.explicitCount || 0) === originalCandidates.length && candidates.length
    ? candidates.length
    : hint?.explicitCount ?? null;
  ...
  return {
    ...
    explicitCount,
    consistency: expectedCount === candidates.length ? "CONSISTENT" : hint?.consistency,
  };
}

const CONFIDENCE_SCORE = Object.freeze({ HIGH: 3, MEDIUM: 2, COUNT_ONLY: 1 });
function landingEvidence(resources, domain) {
  let candidates = resources.flatMap((resource) =>
    inventoryHintsForDomain(resource, domain).map((hint) => ({ resource, hint })));
  if (!candidates.length) return null;

  if (domain === "offers") {
    const offerLandingCandidates = candidates.filter((entry) =>
      String(entry.resource?.classification?.primaryType || "") === "offers");
    if (offerLandingCandidates.length) candidates = offerLandingCandidates;

    const byFamily = new Map();
    for (const entry of candidates) {
      const key = clean(entry.resource?.variantGroupId, 500) || clean(entry.resource?.url, 2_048);
      if (!byFamily.has(key)) byFamily.set(key, []);
      byFamily.get(key).push(entry);
    }
    const families = [...byFamily.values()].sort((left, right) => {
      const leftMax = Math.max(...left.map((entry) => Number(entry.hint.identifiedCount || entry.hint.candidates?.length || 0)));
      const rightMax = Math.max(...right.map((entry) => Number(entry.hint.identifiedCount || entry.hint.candidates?.length || 0)));
      if (leftMax !== rightMax) return rightMax - leftMax;
      if (left.length !== right.length) return right.length - left.length;
      return String(left[0]?.resource?.url || "").localeCompare(String(right[0]?.resource?.url || ""));
    });
    candidates = families[0] || candidates;
  }

  const observedCounts = [...new Set(candidates
  ...
  const representative = [...candidates].sort((left, right) => {
    ...
    if (domain === "offers" && leftNamed !== rightNamed) return rightNamed - leftNamed;
  ...
  const hardConflict = domain === "offers"
    ? representative.hint.consistency === "CONFLICT"
    : strongCounts.length > 1 || candidates.some((entry) => entry.hint.consistency === "CONFLICT");
  ...
  return {
    ...
    observedCounts,
    localizedVariance: domain === "offers" && observedCounts.length > 1,
    authority: domain === "offers" ? "LOCALIZED_OFFER_LANDING_FAMILY" : "LANDING_EVIDENCE",
  };
}
...
function buildDomainInventory(resources, config) {
  ...
  if (config.sourceInventory) {
    // Corporate/legal policy surfaces are retained as supporting provenance but
    // do not create a guest-operational policy expectation. Only explicit
    // operational rules may activate the policies domain.
    const expectationState = detailItems.length ? "DETERMINISTIC" : "ABSENT";
  ...
  if (landing?.localizedVariance) issues.push("localized_offer_inventory_variance");
  if (landing && landing.identifiedCount < landing.expectedCount) issues.push("landing_entities_partially_identified");
  if (landing && detailItems.length > landing.expectedCount && config.key !== "offers") { issues.push("detail_inventory_exceeds_landing_count"); blockingInventoryConflict = true; }
  ...
  const targetCount = config.key === "offers"
    ? landing.expectedCount
    : Math.max(landing.expectedCount, detailItems.length);
  const landingItems = landingExpectedItems(config.key, landing);
  const expectedItems = mergeDetailAndLandingItems(detailItems, landingItems, targetCount).slice(0, targetCount);
  return {
    ...
    evidence: { detailCount: detailItems.length, landingExpectedCount: landing.expectedCount, landingIdentifiedCount: landing.identifiedCount, observedLandingCounts: landing.observedCounts, authority: landing.authority },
  };
}
...
export function buildHotelInventoryV2(siteMap = {}) {
  ...
  const documents = documentResources.map((resource) => ({
    url: clean(resource.url, 2_048), variantGroupId: clean(resource.variantGroupId, 500), domains: inferDocumentDomains(resource), ingestionStatus: "MANUAL",
  }));
  // Public PDFs are discovery metadata only; authoritative documents arrive during onboarding.
  ...
  return {
    ...
    counts: {
      ...
      expectedItems: domains.reduce((sum, domain) => sum + domain.expectedCount, 0), pendingDocuments: 0, manualDocuments: documents.length,
    },
  };
}
