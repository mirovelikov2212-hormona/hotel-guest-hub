...
function spaRole(resource) {
  ...
  if (/(?:^|\/|\b)(?:ceni|prices?|pricing|price-list|catalog|catalogue|doctors?|team|staff|contact)(?:\/|\b|$)/iu.test(text)) return { include: false, entityType: "supporting_resource" };
  if (/(?:pool|pools|wellness-zone|thermal|sauna|steam|relax-zone|басейн|сауна|уелнес зона)/iu.test(text)) return { include: true, entityType: "spa_facility" };
  if (/(?:massage|ritual|treatment|therapy|hydrotherapy|physiotherapy|kinesitherapy|cosmetic-procedures|couples|facial|body|масаж|ритуал|процедур|терап)/iu.test(text)) return { include: true, entityType: "treatment_category" };
  if (/(?:apparatus|device|laser|endosfera|icoone|oscillation|infrared|led-mask|kegel|technology|апарат|лазер|технолог)/iu.test(text)) return { include: true, entityType: "spa_technology" };
  return { include: true, entityType: "spa_surface" };
}
function isOperationalPolicy(resource) {
  return !/(?:privacy|cookie|gdpr|personal-data|защита-на-личните|поверителност)/iu.test(resourceHaystack(resource));
}
...
function landingExpectedItems(domain, evidence) {
  ...
  const entityType = DEFAULT_ENTITY_TYPES[domain] || `${domain}_entity`;
  const items = namedCandidates.slice(0, evidence.expectedCount).map((candidate, index) => {
    ...
    return {
      id: `${domain}:landing:${resource.variantGroupId || resource.url}:${candidateKey}`, domain, entityType,
  ...
  while (items.length < evidence.expectedCount) {
    ...
    items.push({
      id: `${domain}:landing:${resource.variantGroupId || resource.url}:slot-${slot}`, domain, entityType,
