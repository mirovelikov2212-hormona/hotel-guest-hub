...
const GUEST_OPERATIONAL_POLICY = /(?:hotel rules|house rules|guest rules|guest policy|pet policy|pets?|smoking|quiet hours|pool rules|all inclusive rules|terms and conditions|cancellation policy|check-in|check-out|otel kuralları|otel kurallari|misafir kuralları|misafir kurallari|evcil hayvan|sigara|sessiz saat|havuz kuralları|havuz kurallari|iptal koşulları|iptal kosullari|giriş|giris|çıkış|cikis|правила на хотела|домашни любимци|пушене|тихи часове|условия за анулация)/iu;
function isOperationalPolicy(resource) {
  ...
  return /(?:policy|rules|terms|conditions|richtlinien|bedingungen|politici|politikalar|kurallar|koşullar|kosullar|правила|политики|условия)/iu.test(text);
}
