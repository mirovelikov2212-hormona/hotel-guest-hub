...
function landingEvidence(resources, domain) {
  ...
  return { resource: representative.resource, hint: representative.hint, expectedCount: Number(representative.hint.expectedCount || 0), conflict, observedCounts: counts.sort((a, b) => a - b) };
}
...
function landingExpectedItems(domain, evidence) {
  ...
  const items = namedCandidates.slice(0, evidence.expectedCount).map((candidate, index) => {
    ...
    return {
      ...
      languages: unique(resource.languages || []).sort(), crawled: Boolean(resource.crawled), basis: "deterministic_landing_entity",
    };
  });
...
function buildDomainInventory(resources, config) {
  ...
  if (config.logicalSingleton) {
    ...
    return { domain: config.key, expectationState: expectedItems.length ? "DETERMINISTIC" : "ABSENT", expectedCount: expectedItems.length, expectedItems, landingUrls, detailUrls: [], issues: [], evidence: { detailCount: 0, landingExpectedCount: expectedItems.length || null, observedLandingCounts: [] } };
  }
  ...
  if (config.sourceInventory) {
    return { domain: config.key, expectationState: detailItems.length ? "DETERMINISTIC" : surfaceResources.length ? "UNKNOWN" : "ABSENT", expectedCount: detailItems.length, expectedItems: detailItems, landingUrls, detailUrls: detailItems.flatMap((item) => item.urls), issues: [], evidence: { detailCount: detailItems.length, landingExpectedCount: null, observedLandingCounts: [] } };
  }
  ...
  if (!landing) {
    ...
    return { domain: config.key, expectationState, expectedCount: detailItems.length, expectedItems: detailItems, landingUrls, detailUrls: detailItems.flatMap((item) => item.urls), issues, evidence: { detailCount: detailItems.length, landingExpectedCount: null, observedLandingCounts: [] } };
  }
  ...
  return {
    domain: config.key, expectationState: issues.length ? "CONFLICT" : "DETERMINISTIC", expectedCount: targetCount, expectedItems,
    landingUrls, detailUrls: detailItems.flatMap((item) => item.urls), issues,
    evidence: { detailCount: detailItems.length, landingExpectedCount: landing.expectedCount, observedLandingCounts: landing.observedCounts },
  };
}
