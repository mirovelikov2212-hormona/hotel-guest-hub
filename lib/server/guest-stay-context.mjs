...
import { buildOperationalServiceRecoveryState } from "./operational-service-recovery.mjs";
...
function buildFeedbackFacts(items) {
  ...
  for (const item of items) {
    ...
    if (rating !== null && rating <= 3 && clean(status).toLowerCase() !== "fully_resolved") {
      attention.push({
...
export function buildGuestStayContext(input = {}) {
  ...
  const serviceRecovery = buildOperationalServiceRecoveryState({ attention });
