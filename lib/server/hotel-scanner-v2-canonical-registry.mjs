...
const ACCOMMODATION_NON_ENTITY_NAME = /^(?:\d+(?:[.,]\d+)?\s*(?:m²|m2|sq\.?\s*m|sqm)\b|from\s+[€$£]?\s*\d|[€$£]\s*\d|compare\s+rooms?|room\s+compare|room\s+comparison|details?|book(?:\s+now)?)/iu;
const GASTRONOMY_HOTEL_SEO_NAME = /hotel/iu;

const GENERIC_BY_DOMAIN = Object.freeze({
  accommodation: /^(?:accommodation|accommodations|rooms?|our rooms|rooms & suites(?:\s+(?:in|at)\s+.+)?|our rooms & suites(?:\s+(?:in|at)\s+.+)?|room details?|room compare|room comparison|odalar?|konaklama|oda ve süitler|oda ve suitler|настаняване|стаи|нашите стаи|zimmer|zimmer details?|zimmer vergleich|zimmervergleich|unsere zimmer|zimmer\s*&\s*suiten(?:\s+im\s+.+)?|zimmer\s+und\s+suiten(?:\s+im\s+.+)?|unterkunft|cazare|camere|camerele noastre|ubytování|pokoje|номера|размещение)$/iu,
...
const EXPERIENCE_HOTEL_SEO_NAME = /(?:^|\s)(?:hotel|resort|ferienhotel|urlaubshotel|bikehotel|skihotel|wellnesshotel)(?:\s|$)/iu;
...
function candidateAccepted(domain, candidate) {
  ...
  if (domain === "accommodation" && ACCOMMODATION_NON_ENTITY_NAME.test(name)) return false;
  if (domain === "gastronomy" && GASTRONOMY_HOTEL_SEO_NAME.test(name)
    && !GASTRONOMY_ENTITY_NAME.test(name)
    && !GASTRONOMY_PROGRAM_NAME.test(name)) return false;
  if (domain === "experiences" && EXPERIENCE_UMBRELLA_HEADING.test(name)) return false;
  if (domain === "experiences" && EXPERIENCE_HOTEL_SEO_NAME.test(name)) return false;
...
function buildInlineDomain(resources, domain) {
  ...
  const structuralLandingUnion = new Set(allLandings
    .filter((landing) => landing?.authority === "STRUCTURAL")
    .flatMap((landing) => (landing.accepted || []).map((candidate) => entityKey(candidate?.name)).filter(Boolean)));
  const fallbackLandingUnion = new Set(allLandings
    .flatMap((landing) => (landing.accepted || []).map((candidate) => entityKey(candidate?.name)).filter(Boolean)));
  const authoritativeLandingCount = structuralLandingUnion.size || fallbackLandingUnion.size;
  if (domain === "accommodation" && detailItems.length >= 2 && authoritativeLandingCount <= detailItems.length) {
