...
const NAME_RULES = Object.freeze({
  ...
  gastronomy: /(?:restaurant|bar|cafe|café|bistro|dining|grill|lounge|ресторант|бар|кафе|gastst[aä]tte|restaurante?|restaurace|ресторан)/iu,
  services: /(?:kids? corner|children'?s corner|game hall|games room|playroom|hair salon|barbershop|pharmacy|fitness|conference hall|parking|transfer|laundry|business centre|concept store|детски кът|игрална зала|фитнес|фризьор|аптека|конферентна зала|паркинг|трансфер|магазин)/iu,
  experiences: /(?:valley|historical route|route|landmark|museum|park|tourism|trail|hiking|culture|tradition|excursion|attraction|долина|исторически маршрут|маршрут|забележителност|музей|парк|туризъм|пътека|култура|традиц|екскурз)/iu,
});
...
function candidateAccepted(domain, candidate) {
  ...
  if (domain === "spa") return true;
  const rule = NAME_RULES[domain];
  if (!rule) return true;
