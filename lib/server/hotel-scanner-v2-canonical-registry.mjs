...
function buildInlineDomain(resources, domain) {
  ...
  const namedLandingUnion = new Set(allLandings.flatMap((landing) =>
    (landing.accepted || []).map((candidate) => entityKey(candidate?.name)).filter(Boolean)));
  if (domain === "accommodation" && detailItems.length >= 2 && namedLandingUnion.size <= detailItems.length) {
