...
const GENERIC_BY_DOMAIN = Object.freeze({
  ...
  experiences: /^(?:experiences|activities|our experiences|aktiviteler|eğlence|eglence|spor|havuzlar|çocuk aktiviteleri|cocuk aktiviteleri|преживявания|активности|erlebnisse|aktivitäten|experiențe|activități|zážitky|aktivity|впечатления|активности)$/iu,
});
...
function linkedDetailResolution(resources, domain, landingResource, candidate) {
  ...
  for (const [groupId, sourceGroups] of evidenceByGroup) {
    ...
    // A candidate may bind to a detail page only when the detail page itself
    // identifies the same entity. Independent detail discovery below still
    // preserves the real page even when a landing card points at the wrong URL.
...
function inlineDetailInventory(resources, domain) {
  ...
  for (const resource of resources) {
    ...
    // are supporting evidence unless the child title itself is a known hotel
    // object such as Fitness, Pool, Kids Club, Restaurant, Bar, etc.
    if (domain === "experiences" && !semantic) {
      const path = canonicalPageUrl(resource?.url);
      const parent = path.replace(/\/[^/]+$/u, "");
      const parentResource = resources.find((candidate) =>
        candidate?.crawled
        && canonicalPageUrl(candidate?.url) === parent
        && classifyCommonHotelObjectV2(detailDisplayName(candidate), detailDisplayName(candidate), resourcePrimaryDomain(candidate))?.domain === domain);
...
function mergeInlineItems(landingItems, detailItems) {
  ...
  for (const landing of landingItems) {
    ...
    if (detailIndex < 0) {
      if (!merged.some((item) => entityKey(item.nameHint) === entityKey(landing.nameHint))) merged.push(landing);
      continue;
    }
...
function buildInlineDomain(resources, domain) {
  const landings = authoritativeLandings(resources, domain);
  const landingItems = [];
  const landingUrls = [];
  ...
  for (const landing of landings) {
    ...
    landingItems.push(...items.slice(0, clusterExpected));
  ...
  const hasCountConflict = issues.some((issue) => issue.startsWith("canonical_entity_count_exceeds_explicit_count:"));
  return {
    domain,
