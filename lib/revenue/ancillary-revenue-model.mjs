import { readHistoricalServiceIdentitySnapshot } from "../change-management/historical-service-identity.mjs";
...
function requestPriceSnapshot(row) {
  const metadata = requestMetadata(row);
  const snapshot = record(metadata.revenuePriceSnapshot);
  return clean(snapshot.schemaVersion) === "revenue-price-snapshot-v1"
    ? snapshot
    : {};
}

function requestServiceIdentity(row) {
  const metadata = requestMetadata(row);
  const snapshot = readHistoricalServiceIdentitySnapshot(metadata);
  return {
    serviceKey: clean(
      snapshot?.serviceKey
    ...
    title: clean(snapshot?.title || metadata.typeLabel) || null,
    configRevisionId: clean(snapshot?.configRevisionId) || null,
    configSourceChecksum:
      clean(snapshot?.configSourceChecksum).toLowerCase() || null,
    source:
      snapshot
        ? "historical_service_identity_v1"
        : "legacy_request_metadata",
  };
}

function requestServiceKey(row) {
  return requestServiceIdentity(row).serviceKey;
}
...
function requestAmountMinor(row) {
  ...
  const snapshot = requestPriceSnapshot(row);
  return parseRevenueMoneyToMinor(snapshot.price ?? metadata.price);
}
...
function buildBillingLedger(events, fromMs, toMs) {
  ...
  for (const rows of grouped.values()) {
    ...
    for (const row of rows) {
      ...
      const nativeMatchesReconstruction =
        hasNativeDelta && nativeDelta === reconstructedDelta;

      if (hasNativeDelta) {
        nativeEvents += 1;
        if (!nativeMatchesReconstruction) deltaMismatches += 1;
      } else {
      ...
      ledgerEntries.push({
        ...
        revenueDeltaMinor: nativeMatchesReconstruction
          ? nativeDelta
          : reconstructedDelta,
        ledgerSource: nativeMatchesReconstruction
          ? "native_v1"
          : hasNativeDelta
            ? "reconstructed_native_mismatch"
...
export function buildAncillaryRevenueSnapshot(input = {}) {
  ...
  for (const entry of ledger.entries.filter((row) => row.inPeriod)) {
    ...
    const serviceIdentity = request
      ? requestServiceIdentity(request)
      : {
          serviceKey: "unknown",
          title: null,
          configRevisionId: null,
          configSourceChecksum: null,
          source: "missing_request",
        };
    const serviceKey = serviceIdentity.serviceKey;
    ...
    const existing = serviceMap.get(serviceKey) || {
      ...
      serviceTitles: [],
      configRevisionIds: [],
      historicalIdentitySources: [],
      recognizedRevenue: {},
      recognitionEvents: 0,
      reversalEvents: 0,
    };
    if (
      serviceIdentity.title
      && !existing.serviceTitles.includes(serviceIdentity.title)
    ) {
      existing.serviceTitles.push(serviceIdentity.title);
    }
    if (
      serviceIdentity.configRevisionId
      && !existing.configRevisionIds.includes(serviceIdentity.configRevisionId)
    ) {
      existing.configRevisionIds.push(serviceIdentity.configRevisionId);
    }
    if (
      serviceIdentity.source
      && !existing.historicalIdentitySources.includes(serviceIdentity.source)
    ) {
      existing.historicalIdentitySources.push(serviceIdentity.source);
    }
    ...
    services: Object.freeze(
      [...serviceMap.values()]
        .map((item) =>
          Object.freeze({
            ...item,
            serviceTitles: Object.freeze(
              [...item.serviceTitles].sort(),
            ),
            configRevisionIds: Object.freeze(
              [...item.configRevisionIds].sort(),
            ),
            historicalIdentitySources: Object.freeze(
              [...item.historicalIdentitySources].sort(),
            ),
