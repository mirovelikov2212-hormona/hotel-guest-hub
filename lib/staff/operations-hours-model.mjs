...
export function isDepartmentWorkingHoursForConfig(input) {
  ...
  if (config?.departmentRoutingRuntimeActivated !== true) {
    return isDepartmentWorkingHours(date, DEFAULT_HOTEL_TIME_ZONE);
  }
  ...
  let localMinutes;
  try {
    localMinutes = getHotelLocalMinutes(
      date,
      normalizeText(config.hotelTimezone),
    );
  } catch {
    return false;
  }
