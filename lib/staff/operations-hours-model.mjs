...
function sameRuleAuthority(left, right) {
  return Boolean(
    left
    && right
...
function isWeeklyRuleActive(rule, localDay, localMinutes, allowPreviousDaySpill) {
  ...
  return rule.windows.some((window) => {
    ...
    if (window.days.includes(localDay) && localMinutes >= window.open) return true;
    return Boolean(
      allowPreviousDaySpill
...
export function isDepartmentWorkingHoursForConfig(input) {
  ...
  if (schedule) {
    ...
    const allowPreviousDaySpill = sameRuleAuthority(
      currentAuthority,
      previousAuthority,
    );

    return isWeeklyRuleActive(
