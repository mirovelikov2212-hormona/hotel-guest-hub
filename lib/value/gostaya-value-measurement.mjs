...
export function normalizeGostayaValueBaseline(input) {
  ...
  const goLiveAt = iso(input?.goLiveAt, "VALUE_BASELINE_GOLIVE_INVALID");
  if (Date.parse(baselineTo) > Date.parse(goLiveAt)) {
...
function directDepartmentMetrics(requests, baseline, fromMs, toMs) {
  const periodRequests = (Array.isArray(requests) ? requests : []).filter(
    (row) =>
      row?.is_test !== true
  ...
  const attributableBypassRequests =
...
function serviceRecoveryMetrics(events, baseline, fromMs, toMs) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    if (!requestId || time === null) continue;
...
export function buildGostayaValueMeasurement(input = {}) {
  ...
  if (fromMs >= toMs || fromMs < Date.parse(baseline.goLiveAt)) {
