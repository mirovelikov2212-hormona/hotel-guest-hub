...
export function collectVisibleHotelOfferAssetIds(offers, options = {}) {
  ...
  for (const offer of getVisibleHotelOffers(offers, options)) {
    ...
    for (const creative of Object.values(assets.readyCreativeByLang || {})) {
      if (creative?.assetId) ids.add(text(creative.assetId).toLowerCase());
    }
  }
...
export function getHotelOfferReadyCreative(offer, language) {
  const creatives = offer?.assets?.readyCreativeByLang;
  if (!creatives || typeof creatives !== "object" || Array.isArray(creatives)) return null;

  const locale = text(language).toLowerCase();
  const base = locale.split("-")[0];
  const candidates = [locale, base, "en", "default"];
  for (const key of candidates) {
    const creative = creatives[key];
    if (creative?.assetId && (creative.kind === "image" || creative.kind === "document")) {
      return {
        assetId: text(creative.assetId).toLowerCase(),
        kind: creative.kind,
        language: key,
      };
    }
