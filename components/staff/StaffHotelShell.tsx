...
export default function StaffHotelShell({ hotelSlug, brand, children }: { hotelSlug: string; brand: StaffHotelBrand; children: ReactNode }) {
  ...
  return (
    <div className="stayhub-staff-shell min-h-screen" data-staff-theme={theme} data-brand-source={brand.source} style={style}>
        ...
        <header className="stayhub-staff-topbar mb-5 overflow-hidden rounded-3xl border p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><span className="stayhub-staff-brand-dot" aria-hidden="true" /><p className="text-xs font-semibold uppercase tracking-[0.22em] opacity-70">StayHub Staff</p></div>
              <h1 className="mt-1 truncate text-xl font-semibold tracking-tight sm:text-2xl">{brand.hotelName}</h1>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select value={lang} onChange={(event) => setLang(event.target.value as typeof lang)} className="stayhub-staff-select rounded-xl border px-3 py-2 text-sm outline-none" aria-label="Staff UI language">
