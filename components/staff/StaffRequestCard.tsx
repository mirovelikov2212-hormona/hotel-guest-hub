...
import StaffRequestConversationPanel from "@/components/staff/StaffRequestConversationPanel";
...
function getStaffRequestIcon(type: string): string {
  switch (type) {
    ...
    case "extra_pillow": return "🛏️";
    case "extra_blanket": return "🧣";
    case "bathrobe": return "🧥";
    case "slippers": return "🩴";
    case "baby_cot": return "🍼";
    case "iron": return "🧼";
    case "minibar": return "🥤";
    case "laundry": return "🧺";
    case "late_checkout": return "🕒";
    case "taxi": return "🚕";
    case "wake_up_call": return "⏰";
    case "information_request":
    case "information": return "ℹ️";
    case "reservation_help": return "📋";
    case "other_reception": return "🛎️";
    case "air_conditioning": return "❄️";
    case "light_not_working": return "💡";
    case "no_hot_water": return "🚿";
    case "tv_issue": return "📺";
    case "bathroom_issue": return "🛁";
    case "door_lock_issue": return "🚪";
    case "wifi_issue": return "📶";
    case "power_outlet_issue": return "🔌";
    case "safe_issue": return "🔒";
    case "balcony_door_issue": return "🚪";
    case "minibar_not_cooling": return "🧊";
    case "other_technical_issue": return "🛠️";
    case "restaurant_reservation": return "🍽️";
    case "massage_booking": return "💆";
    default: return "•";
  }
}
...
function getBillingBadgeClasses(status: StaffBillingStatus) {
  switch (status) {
    case "charged": return "border-emerald-400/30 bg-emerald-400/15 text-emerald-100";
    case "waived": return "border-sky-400/30 bg-sky-400/15 text-sky-100";
    case "cancelled": return "border-rose-400/30 bg-rose-400/15 text-rose-100";
    case "pending":
    default: return "border-amber-400/30 bg-amber-400/15 text-amber-100";
  }
}
...
export default function StaffRequestCard({
  ...
  const shouldShowBillingActions = shouldShowBilling && canCharge && isPendingBilling && billingActionsOpen;
  const shouldShowBillingToggle = shouldShowBilling && canCharge && isPendingBilling;
  const shouldShowReturn = canAct && isNew && !shouldUseBillingOnlyFlow && !((mode === "reception" || mode === "manager") && request.requiresBilling);
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {mode !== "department" ? (
              <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${getStaffDepartmentClass(request.department)}`}>
                {translateDepartment(request.department, lang)}
              </span>
            ) : null}
            <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${staffStatusClasses[request.status]}`}>
              {translateStaffStatus(request.status, lang)}
            </span>
            {shouldShowBilling ? (
              <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${getBillingBadgeClasses(billingStatus)}`}>
                {billingLabel}{billingAmount ? ` · ${billingAmount}` : ""}
              </span>
            ) : null}
            {isOverdue ? (
          ...
          <div>
            <div className="flex items-center gap-3">
              <span className="text-2xl leading-none">{getStaffRequestIcon(request.type)}</span>
              <h3 className="text-2xl font-semibold tracking-tight text-white">{cleanRequestTitle(request.typeLabel)}</h3>
            </div>
            <p className="mt-1 text-sm text-white/50">
              {t.requestedAt} {formatRequestDateTime(request.createdAtIso, lang, hotelTimeZone)}
            </p>
          </div>
        ...
        <div className="flex w-full flex-col gap-3 lg:w-72">
          <StaffRequestConversationPanel request={request} />
          ...
          {shouldShowBillingToggle ? (
            ...
            >
              {t.billingActions}{billingAmount ? ` · ${billingAmount}` : ""}
            </button>
          ) : null}

          {shouldShowBillingActions ? (
            <div className="grid gap-2 rounded-2xl border border-amber-300/20 bg-amber-400/5 p-3">
              <button type="button" onClick={() => onCharge?.(request.id)} className="min-h-14 rounded-2xl bg-amber-500 px-4 text-base font-semibold text-white transition hover:bg-amber-400">
                {t.charge}{billingAmount ? ` · ${billingAmount}` : ""}
              </button>
              <button type="button" onClick={() => onWaive?.(request.id)} className="min-h-12 rounded-2xl border border-sky-300/30 bg-sky-400/15 px-4 text-sm font-semibold text-sky-100 transition hover:bg-sky-400/25">
                {t.noCharge}
              </button>
              <button type="button" onClick={() => onCancelBilling?.(request.id)} className="min-h-12 rounded-2xl border border-rose-400/30 bg-rose-400/15 px-4 text-sm font-semibold text-rose-100 transition hover:bg-rose-400/25">
                {t.cancelPaidService}
              </button>
            </div>
          ) : null}
          ...
          {canAct && isNew && !shouldUseBillingOnlyFlow ? (
            <>
              <button type="button" onClick={() => onStart?.(request.id)} className="min-h-14 rounded-2xl bg-sky-500 px-4 text-base font-semibold text-white transition hover:bg-sky-400">
                {t.start}
              </button>
              {shouldShowReturn ? (
                <button type="button" onClick={() => onReturn?.(request.id)} className="min-h-14 rounded-2xl border border-rose-400/30 bg-rose-400/15 px-4 text-base font-semibold text-rose-100 transition hover:bg-rose-400/25">
                  {t.return}
                </button>
              ) : null}
            </>
          ...
          {canAct && isInProgress && !shouldUseBillingOnlyFlow ? (
            <button type="button" onClick={() => onDone?.(request.id)} className="min-h-14 rounded-2xl bg-emerald-500 px-4 text-base font-semibold text-white transition hover:bg-emerald-400">
              {t.done}
            </button>
          ) : null}
