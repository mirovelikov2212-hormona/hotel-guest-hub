...
const COPY: Record<LangKey, {
  label: string; title: string; empty: string; close: string; from: string; important: string;
  reception: string; you: string; reply: string; placeholder: string; send: string; sending: string; replyError: string;
}> = {
  bg: { label: "Съобщения", title: "Съобщения от хотела", empty: "Няма нови съобщения.", close: "Затвори", from: "От", important: "Важно", reception: "Рецепция", you: "Вие", reply: "Отговор до рецепция", placeholder: "Напишете отговор…", send: "Изпрати", sending: "Изпращане…", replyError: "Отговорът не беше изпратен. Опитайте отново." },
  en: { label: "Messages", title: "Hotel messages", empty: "There are no new messages.", close: "Close", from: "From", important: "Important", reception: "Reception", you: "You", reply: "Reply to reception", placeholder: "Write a reply…", send: "Send", sending: "Sending…", replyError: "The reply could not be sent. Please try again." },
  de: { label: "Mitteilungen", title: "Hotel-Mitteilungen", empty: "Keine neuen Mitteilungen.", close: "Schließen", from: "Von", important: "Wichtig", reception: "Rezeption", you: "Sie", reply: "Antwort an die Rezeption", placeholder: "Antwort schreiben…", send: "Senden", sending: "Wird gesendet…", replyError: "Die Antwort konnte nicht gesendet werden. Bitte erneut versuchen." },
  ro: { label: "Mesaje", title: "Mesaje de la hotel", empty: "Nu există mesaje noi.", close: "Închide", from: "De la", important: "Important", reception: "Recepție", you: "Dvs.", reply: "Răspuns către recepție", placeholder: "Scrieți un răspuns…", send: "Trimite", sending: "Se trimite…", replyError: "Răspunsul nu a putut fi trimis. Încercați din nou." },
  cs: { label: "Zprávy", title: "Zprávy z hotelu", empty: "Žádné nové zprávy.", close: "Zavřít", from: "Od", important: "Důležité", reception: "Recepce", you: "Vy", reply: "Odpověď recepci", placeholder: "Napište odpověď…", send: "Odeslat", sending: "Odesílání…", replyError: "Odpověď se nepodařilo odeslat. Zkuste to znovu." },
  ru: { label: "Сообщения", title: "Сообщения от отеля", empty: "Новых сообщений нет.", close: "Закрыть", from: "От", important: "Важно", reception: "Рецепция", you: "Вы", reply: "Ответить на ресепшен", placeholder: "Напишите ответ…", send: "Отправить", sending: "Отправка…", replyError: "Не удалось отправить ответ. Попробуйте ещё раз." },
};
...
function readLanguage(defaultLanguage: LangKey): LangKey {
  ...
  try { return normalizeLanguage(window.localStorage.getItem("stayhub_guest_language") || defaultLanguage); }
...
function formatTimestamp(value: string | null | undefined, language: LangKey) {
  if (!value) return "";
...
export default function GuestCommunicationsInbox({ hotelSlug, defaultLanguage, brandColor }: { hotelSlug: string; defaultLanguage: LangKey; brandColor: string }) {
  ...
  const loadMessages = useCallback(async (identity: StoredGuestRoomState | null, lang: LangKey) => {
    if (!identity?.stayId || !identity.stayDeviceId || !identity.deviceToken) {
      setMessages([]); setDirectReplyEnabled(false); return;
    }
    try {
      const response = await fetch("/api/guest/communications", {
        ...
        body: JSON.stringify({ hotelSlug, stayId: identity.stayId, stayDeviceId: identity.stayDeviceId, deviceToken: identity.deviceToken, language: lang }),
      });
      if (!response.ok) return;
      const payload = await response.json() as { messages?: GuestCommunication[]; directReplyEnabled?: boolean };
      setMessages(Array.isArray(payload.messages) ? payload.messages : []);
      setDirectReplyEnabled(Boolean(payload.directReplyEnabled));
    } catch (error) { console.warn("Guest communications inbox refresh failed", error); }
  }, [hotelSlug]);
  ...
  useEffect(() => {
    ...
    return () => { window.clearInterval(identityTimer); window.removeEventListener("focus", onFocus); };
  }, [refreshIdentity]);
  ...
  async function sendReply() {
    ...
    try {
      const response = await fetch("/api/guest/communications", {
        method: "POST", credentials: "include", cache: "no-store", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "reply", hotelSlug, stayId: stay.stayId, stayDeviceId: stay.stayDeviceId, deviceToken: stay.deviceToken, language, body: reply.trim() }),
      });
        ...
        <section className="mb-3 flex max-h-[min(76vh,40rem)] w-[min(calc(100vw-2rem),24rem)] flex-col overflow-hidden rounded-3xl border border-black/10 bg-white text-[#163033] shadow-2xl">
          ...
          <div className="space-y-3 overflow-y-auto p-4">
            {messages.length ? messages.map((message) => {
              const direct = message.audienceType === "direct_guest";
              const fromGuest = direct && message.senderType === "guest";
              return (
                <article key={message.id} className={`rounded-2xl border p-4 ${fromGuest ? "ml-6 border-black/10 bg-[#eef7f3]" : message.category === "emergency" ? "border-red-300 bg-red-50" : "border-black/10 bg-[#f7faf9]"}`}>
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    ...
                    <span className="opacity-55">{copy.from}: {fromGuest ? copy.you : message.department?.name || (direct ? copy.reception : "StayHub")}</span>
                    <span className="ml-auto opacity-45">{formatTimestamp(message.sentAt || message.displayFrom || message.createdAt, language)}</span>
                  </div>
          ...
          {directReplyEnabled ? (
            <div className="border-t border-black/10 p-4">
              <p className="mb-2 text-xs font-medium opacity-60">{copy.reply}</p>
              <textarea value={reply} onChange={(event) => setReply(event.target.value)} maxLength={1000} rows={3} placeholder={copy.placeholder} className="w-full resize-none rounded-2xl border border-black/10 bg-black/[0.03] px-3 py-2.5 text-sm outline-none" />
              {replyError ? <p className="mt-2 text-xs text-red-700">{replyError}</p> : null}
              <button type="button" disabled={!reply.trim() || sending} onClick={() => void sendReply()} className="mt-2 w-full rounded-xl px-4 py-2.5 text-sm font-medium disabled:opacity-45" style={{ background: "var(--guest-message-brand)", color: "#102027" }}>{sending ? copy.sending : copy.send}</button>
            </div>
          ) : null}
        </section>
      ) : null}
