...
export default function GuestHub({ config }: { config: HotelConfig }) {
  ...
  const openInfoDialog = useCallback((message: string, title?: string) => {
    setMessageDialog({
      mode: "info",
      title: title || config.hotelName || "StayHub",
      message,
    });
  }, [config.hotelName]);

  const openConfirmDialog = useCallback((message: string, title?: string) => {
  ...
  const closeMessageDialog = useCallback((result: boolean) => {
  ...
  const ensureConfirmedRoom = () => {
    ...
    openInfoDialog(roomCopy.lockedActionAlert);
    return false;
  };
  ...
  const confirmManualRoom = async () => {
    ...
    if (!candidate) {
      openInfoDialog(roomCopy.missingRoomAlert);
      return;
    }
  ...
  const hiddenGuestRequestIds = useMemo(
    () => new Set(["room_cleaning_request", "extra_cleaning", "cleaning"]),
    []
  );
  ...
  const requestDefs = useMemo(
    ...
    [config.requestDefs, hiddenGuestRequestIds]
  );
  ...
  const aiServices = useMemo(() => {
    ...
    const copy = {
      bg: {
        ...
        lateCheckout:
          lateCheckoutInfo ||
          "Късният check-out се предлага срещу допълнително заплащане и се потвърждава от рецепция.",
      ...
      en: {
        ...
        lateCheckout:
          lateCheckoutInfo ||
          "Late checkout is available for an additional charge and is confirmed by reception.",
      ...
      de: {
        ...
        lateCheckout:
          lateCheckoutInfo ||
          "Late Check-out wird gegen Aufpreis angeboten und von der Rezeption bestätigt.",
  ...
  function promptRequestQuantity(def: RequestDef) {
    ...
    if (!Number.isFinite(qty) || qty < min || qty > max) {
      openInfoDialog(String(tUI("request_quantity_invalid") || `Please enter a number between ${min} and ${max}.`));
      return null;
    }
    ...
    return defs
      ...
      .map((def) => {
        ...
        return {
          ...
          kind: "link" as const,
          onClick: () => { void handleRequestDefClick(def); },
        };
      });
  }
  ...
  const submitGuestRequest = async ({
    ...
    if (!roomValue) {
      openInfoDialog(roomCopy.missingRoomQrAlert);
      return;
    }
    ...
    if (!hotelScopeReady || !hotelId) {
      openInfoDialog(roomCopy.requestFailed);
      return;
    }
    ...
    try {
      ...
      const housekeepingTypes = new Set<StaffRequestType>([
        "towels",
        "toilet_paper",
        "extra_pillow",
        "extra_blanket",
      ...
      const shouldRouteHousekeepingToReception =
        housekeepingTypes.has(type) &&
        serviceTime !== "tomorrow" &&
        housekeepingRoutedToReception;

      const departmentOverride = shouldRouteHousekeepingToReception
        ? "reception"
        : undefined;
          ...
          ? [
              {
                ...
                onClick: async () => {
                  if (!ensureConfirmedRoom()) return;
                  if (!(await confirmInfoBlock(lateCheckoutInfo))) return;
          ...
          .map((x) => {
            ...
            if (action?.mode === "info") {
              return {
                ...
                onClick: () => {
                  if (!ensureConfirmedRoom()) return;
                  openInfoDialog(action.getMessage(lang), String(tUI(x.labelKey) || x.key));
                },
              };
            }
              ...
              return {
                ...
                onClick: async () => {
                  ...
                  if (x.key === "minibar" && minibarNotice) {
                    if (!(await confirmInfoBlock(minibarNotice))) return;
                  }
...
function MessageDialog({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-neutral-950 text-white shadow-2xl">
        <div className="border-b border-white/10 px-5 py-4">
          <div className="text-lg font-semibold">{title}</div>
        </div>
        <div className="px-5 py-4">
          <div className="whitespace-pre-wrap text-sm leading-6 text-neutral-200">{message}</div>
        </div>
        <div className="flex items-center justify-end gap-3 border-t border-white/10 px-5 py-4">
          ...
          <button
            ...
            className="rounded-xl bg-[#9B86BD] px-4 py-2 text-sm font-semibold text-[#0D1B2A] transition hover:opacity-95"
          >
            {mode === "confirm" ? "Continue" : "OK"}
          </button>
        </div>
      </div>
    </div>
