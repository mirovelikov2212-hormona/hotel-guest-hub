...
import { getStaffSessionCookieName } from "@/lib/staff-auth/cookie-name";
...
export function middleware(req: NextRequest) {
  ...
  const cookieName = getStaffSessionCookieName(hotelSlug, role as any);
  const hasSessionCookie = Boolean(
    req.cookies.get(cookieName)?.value || req.cookies.get("stayhub_staff_session")?.value
  );
  if (hasSessionCookie) {
    return NextResponse.next();
  }
