...
create table if not exists public.factory_sandbox_certification_runs (
  ...
  unique (envelope_projection_run_id)
);

create index if not exists factory_sandbox_certification_runs_actor_idx
  on public.factory_sandbox_certification_runs(actor_admin_id);
create index if not exists factory_sandbox_certification_runs_production_hotel_idx
  on public.factory_sandbox_certification_runs(production_hotel_id);
create index if not exists factory_sandbox_certification_runs_sandbox_hotel_idx
  on public.factory_sandbox_certification_runs(sandbox_hotel_id);
create index if not exists factory_sandbox_certification_runs_production_revision_idx
  on public.factory_sandbox_certification_runs(production_revision_id);
create index if not exists factory_sandbox_certification_runs_sandbox_revision_idx
  on public.factory_sandbox_certification_runs(sandbox_revision_id);

alter table public.factory_sandbox_certification_runs enable row level security;

revoke all on table public.factory_sandbox_certification_runs from public, anon, authenticated;
revoke all on table public.factory_sandbox_certification_runs from service_role;
...
comment on table public.factory_sandbox_certification_runs is
  'Immutable P2.5 audit ledger for exact Product Factory Sandbox certification. Certification may activate only the Sandbox hotel runtime gate; Production remains fail-closed.';
...
declare
  ...
  v_sandbox_is_sandbox boolean;
  v_sandbox_production_hotel_id uuid;
  v_production_is_sandbox boolean;
...
begin
  ...
  if p_evidence_hash !~ '^[a-f0-9]{64}$' then
    raise exception 'P2_5_EVIDENCE_HASH_INVALID';
  end if;

  if p_checks is null or jsonb_typeof(p_checks) <> 'object' then
    raise exception 'P2_5_CHECKS_INVALID';
  end if;
  ...
  foreach v_required_check in array array[
    ...
    if p_checks->v_required_check is distinct from 'true'::jsonb then
      raise exception 'P2_5_REQUIRED_CHECK_NOT_PASSED:%', v_required_check;
    end if;
  end loop;

  perform pg_advisory_xact_lock(
    hashtextextended('stayhub:p2.5:sandbox-certification:' || p_envelope_projection_run_id::text, 0)
  );

  select * into v_envelope
  from public.factory_onboarding_envelope_projection_runs
  where id = p_envelope_projection_run_id
  for update;
  if not found or v_envelope.status <> 'completed' then
    raise exception 'P2_5_ENVELOPE_PROJECTION_RUN_INVALID';
  end if;
  ...
  if found then
    ...
    return query select v_existing.id, v_existing.sandbox_hotel_id,
  ...
  if not found or v_operational.status <> 'completed' then
    raise exception 'P2_5_OPERATIONAL_PROJECTION_RUN_INVALID';
  end if;

  select * into v_core
  from public.factory_core_resource_projection_runs
  where id = v_operational.core_projection_run_id;
  if not found or v_core.status <> 'completed' then
    raise exception 'P2_5_CORE_PROJECTION_RUN_INVALID';
  end if;
  ...
  if not found or v_onboarding.status <> 'completed' then
    raise exception 'P2_5_ONBOARDING_RUN_INVALID';
  end if;

  if v_envelope.production_revision_id <> v_onboarding.production_revision_id + interval '0 seconds' then
    null;
  end if;

  if v_envelope.production_revision_id <> v_operational.production_revision_id
     and v_envelope.production_revision_id = v_operational.production_revision_id then
    raise exception 'P2_5_UNREACHABLE_LINEAGE_GUARD';
  end if;
  ...
  if not exists (
    ...
    raise exception 'P2_5_ENVELOPE_REVISION_INVALID';
  end if;

  select active, is_sandbox
    into v_production_active, v_production_is_sandbox
  from public.hotels
  where id = v_onboarding.production_hotel_id
  for update;

  select active, is_sandbox, production_hotel_id
    into v_sandbox_active, v_sandbox_is_sandbox, v_sandbox_production_hotel_id
  from public.hotels
  where id = v_onboarding.sandbox_hotel_id
  for update;

  if v_production_active is distinct from false
     or v_production_is_sandbox is distinct from false
     or v_sandbox_active is distinct from false
     or v_sandbox_is_sandbox is distinct from true
     or v_sandbox_production_hotel_id is distinct from v_onboarding.production_hotel_id then
    raise exception 'P2_5_FACTORY_ENVIRONMENT_STATE_INVALID';
  end if;
  ...
  if not exists (
    ...
    raise exception 'P2_5_PROPERTY_ENVIRONMENT_MAPPING_INVALID';
  end if;

  if not exists (
    select 1 from public.hotel_role_templates rt
    where rt.hotel_id = v_onboarding.sandbox_hotel_id
    ...
    raise exception 'P2_5_MANAGER_ROLE_TEMPLATE_MISSING';
  end if;
  ...
  if exists (
    ...
    raise exception 'P2_5_DEPARTMENT_ROLE_TEMPLATE_MISSING';
  end if;

  if exists (
    select 1 from public.hotel_role_templates rt
    where rt.hotel_id in (v_onboarding.production_hotel_id, v_onboarding.sandbox_hotel_id)
      and (rt.runtime_enabled <> false
        or rt.permissions_json->>'configured' <> 'false'
        or jsonb_array_length(rt.permissions_json->'permissions') <> 0)
  ) then
    raise exception 'P2_5_ROLE_TEMPLATE_NOT_FAIL_CLOSED';
  end if;

  if exists (
    select 1 from public.hotel_service_definitions s
    where s.hotel_id in (v_onboarding.production_hotel_id, v_onboarding.sandbox_hotel_id)
      and s.runtime_enabled = true
  ) or exists (
    select 1 from public.hotel_workflow_definitions w
    where w.hotel_id in (v_onboarding.production_hotel_id, v_onboarding.sandbox_hotel_id)
      and w.runtime_enabled = true
  ) or exists (
    select 1 from public.routing_rules rr
    where rr.hotel_id in (v_onboarding.production_hotel_id, v_onboarding.sandbox_hotel_id)
      and rr.active = true
  ) then
    raise exception 'P2_5_OPERATIONAL_RUNTIME_NOT_FAIL_CLOSED';
  end if;

  if exists (
    select 1 from public.hotel_integration_configs i
    where i.hotel_id in (v_onboarding.production_hotel_id, v_onboarding.sandbox_hotel_id)
      and i.status <> 'placeholder'
  ) then
    raise exception 'P2_5_INTEGRATION_NOT_PLACEHOLDER';
  end if;

  if not exists (
    select 1 from public.hotel_reporting_configs r
    where r.hotel_id = v_onboarding.sandbox_hotel_id
      and r.enabled = false
      and jsonb_typeof(r.recipients_json) = 'array'
      and jsonb_array_length(r.recipients_json) = 0
  ) then
    raise exception 'P2_5_REPORTING_NOT_FAIL_CLOSED';
  end if;

  if not exists (
    select 1 from public.hotel_branding_configs b
    where b.hotel_id = v_onboarding.sandbox_hotel_id and b.status = 'placeholder'
  ) or not exists (
    select 1 from public.hotel_knowledge_configs k
    where k.hotel_id = v_onboarding.sandbox_hotel_id and k.status = 'placeholder'
  ) then
    raise exception 'P2_5_CONTENT_PLACEHOLDERS_INVALID';
  end if;

  if not exists (
    select 1 from public.hotel_ai_permission_configs a
    where a.hotel_id = v_onboarding.sandbox_hotel_id
    ...
    raise exception 'P2_5_AI_PERMISSIONS_NOT_FAIL_CLOSED';
  end if;
  ...
  if not exists (
    ...
    select 1 from public.hotel_public_identity_configs i
    where i.hotel_id = v_onboarding.sandbox_hotel_id and i.status = 'reserved'
  ) then
    raise exception 'P2_5_PUBLIC_IDENTITIES_NOT_RESERVED';
  end if;

  if not exists (
    select 1 from public.hotel_health_certification_state h
    where h.hotel_id = v_onboarding.production_hotel_id
      and h.status = 'pending' and h.certification_status = 'not_started'
  ) or not exists (
    select 1 from public.hotel_health_certification_state h
    where h.hotel_id = v_onboarding.sandbox_hotel_id
      and h.status = 'pending' and h.certification_status = 'not_started'
  ) then
    raise exception 'P2_5_HEALTH_STATE_INVALID';
  end if;
  ...
  set status = 'certified', updated_at = v_now
  where hotel_id = v_onboarding.sandbox_hotel_id;
  ...
  set validation_json = jsonb_build_object(
        ...
        'warnings', jsonb_build_array('P2_5_SANDBOX_CERTIFIED')
      )
  where id = v_envelope.sandbox_revision_id
    and hotel_id = v_onboarding.sandbox_hotel_id;
  ...
  set last_verified_at = v_now,
      metadata_json = coalesce(metadata_json,'{}'::jsonb) || jsonb_build_object(
        ...
        'certificationEvidenceHash',p_evidence_hash,
  ...
  where id = v_onboarding.sandbox_hotel_id
    and active = false
    and is_sandbox = true
    and production_hotel_id = v_onboarding.production_hotel_id;
  if not found then
    raise exception 'P2_5_SANDBOX_ACTIVATION_FAILED';
  end if;
  ...
  if exists (
    ...
    select 1 from public.hotel_health_certification_state h
    where h.hotel_id = v_onboarding.production_hotel_id
      and (h.status <> 'pending' or h.certification_status <> 'not_started')
  ) then
    raise exception 'P2_5_PRODUCTION_STATE_CHANGED';
  end if;

  insert into public.factory_sandbox_certification_runs (
    envelope_projection_run_id, actor_admin_id,
    production_hotel_id, sandbox_hotel_id,
    production_revision_id, sandbox_revision_id,
    evidence_hash, checks_json, status
  ) values (
    p_envelope_projection_run_id, p_actor_admin_id,
    v_onboarding.production_hotel_id, v_onboarding.sandbox_hotel_id,
    v_envelope.production_revision_id, v_envelope.sandbox_revision_id,
    p_evidence_hash, p_checks, 'passed'
  ...
  insert into public.control_plane_audit_log (
    ...
    'factory_sandbox_certified',
    'factory_sandbox_certification_run',
    v_run_id::text,
    jsonb_build_object(
      'stage','p2.5',
      'envelopeProjectionRunId',p_envelope_projection_run_id,
      'evidenceHash',p_evidence_hash,
      'sandboxActive',true,
      'sandboxIdentityStatus','certified',
      'sandboxCertificationStatus','passed',
      'productionActive',false,
      'productionIdentityStatus','reserved',
      'productionPublication','not_started'
    )
  );

  return query select v_run_id, v_onboarding.sandbox_hotel_id,
    v_envelope.sandbox_revision_id, false;
end;
$function$;

revoke all on function public.certify_factory_sandbox_v1(uuid,uuid,text,jsonb)
  from public, anon, authenticated;
grant execute on function public.certify_factory_sandbox_v1(uuid,uuid,text,jsonb)
  to service_role;

comment on function public.certify_factory_sandbox_v1(uuid,uuid,text,jsonb) is
  'P2.5 audited Sandbox-only certification. Validates exact P2.4 lineage and fail-closed resources, certifies/activates only the Sandbox hotel, and leaves Production inactive/reserved.';
