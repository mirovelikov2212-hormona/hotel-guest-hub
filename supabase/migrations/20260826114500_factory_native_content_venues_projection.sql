...
create table if not exists public.factory_native_content_projection_runs (
  ...
  native_resources_hash text not null,
  ...
  sandbox_hotel_id uuid not null
    references public.hotels(id) on delete restrict,
  hotel_info_items_count integer not null,
  venues_count integer not null,
  status text not null default 'completed',
  created_at timestamptz not null default now(),
  constraint factory_native_content_projection_operational_unique
    unique (operational_projection_run_id),
  constraint factory_native_content_projection_hash_check
    check (native_resources_hash ~ '^[a-f0-9]{64}$'),
  constraint factory_native_content_projection_counts_check
    check (hotel_info_items_count >= 0 and venues_count >= 0),
  constraint factory_native_content_projection_status_check
    check (status = 'completed')
);

create index if not exists factory_native_content_projection_actor_idx
  on public.factory_native_content_projection_runs (actor_admin_id, created_at desc);
create index if not exists factory_native_content_projection_production_idx
  on public.factory_native_content_projection_runs (production_hotel_id, created_at desc);
create index if not exists factory_native_content_projection_sandbox_idx
  on public.factory_native_content_projection_runs (sandbox_hotel_id, created_at desc);

alter table public.factory_native_content_projection_runs enable row level security;
revoke all on table public.factory_native_content_projection_runs from anon, authenticated;
revoke all on table public.factory_native_content_projection_runs from service_role;
grant select, insert on table public.factory_native_content_projection_runs to service_role;

alter table public.hotel_knowledge_configs
  add column if not exists factory_managed boolean not null default false,
  add column if not exists factory_projection_run_id uuid
    references public.factory_native_content_projection_runs(id) on delete restrict;

create index if not exists hotel_knowledge_configs_factory_projection_idx
  on public.hotel_knowledge_configs (factory_projection_run_id)
  where factory_projection_run_id is not null;

alter table public.venues
  add column if not exists factory_managed boolean not null default false,
  add column if not exists factory_source_key text,
  add column if not exists factory_type_key text,
  add column if not exists factory_payload_json jsonb,
  add column if not exists factory_projection_run_id uuid
    references public.factory_native_content_projection_runs(id) on delete restrict;

create unique index if not exists venues_hotel_factory_source_unique
  on public.venues (hotel_id, factory_source_key)
  where factory_source_key is not null;
create index if not exists venues_factory_projection_idx
  on public.venues (factory_projection_run_id)
  where factory_projection_run_id is not null;

alter table public.venues
  add constraint venues_factory_source_key_check
    check (
      factory_source_key is null
      or factory_source_key ~ '^[a-z][a-z0-9_-]{0,62}$'
    ),
  add constraint venues_factory_type_key_check
    check (
      factory_type_key is null
      or factory_type_key ~ '^[a-z][a-z0-9_-]{0,62}$'
    ),
  add constraint venues_factory_payload_json_check
    check (
      factory_payload_json is null
      or jsonb_typeof(factory_payload_json) = 'object'
    ),
  add constraint venues_factory_ownership_check
    check (
      factory_managed = false
      or (
        factory_source_key is not null
        and factory_type_key is not null
        and factory_payload_json is not null
        and factory_projection_run_id is not null
      )
    );
...
grant execute on function public.project_factory_native_content_venues_v1(
  uuid, uuid, text, text, text, jsonb
) to service_role;

comment on table public.factory_native_content_projection_runs is
  'Immutable Factory STEP 2C native-content/venue projection lineage. Projection does not activate hotels or venues.';
comment on column public.hotel_knowledge_configs.factory_managed is
  'True only for knowledge rows owned by Product Factory projection. Legacy rows remain false and are never overwritten by Factory.';
comment on column public.venues.factory_source_key is
  'Stable Product Factory venue key. Null for legacy/manual venue rows.';
comment on column public.venues.factory_payload_json is
  'Full multilingual Product Factory venue payload; relational enum/type columns remain backward-compatible projections.';
