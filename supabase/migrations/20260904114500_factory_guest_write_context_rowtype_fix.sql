-- Fix composite-record assignment in the consolidated Factory guest write RPC.
-- `select s into %rowtype` returns one composite value; selecting `s.*` assigns
-- table columns to the rowtype fields as intended.
