...
begin
  ...
  loop
    -- One central hotel-time boundary is used by both Guest Hub reads and the
    -- atomic booking RPC. A start that is no longer in the future must never
    -- be shown or accepted, regardless of the caller's timezone.
...
comment on function public.get_massage_runtime_available_times(uuid, text, date, text) is
  'Tenant-scoped native massage availability. M14.3.3 also rejects same-day starts that are no longer in the future in the configured hotel schedule timezone.';
