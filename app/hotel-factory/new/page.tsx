...
export default async function HotelFactoryNewPage({
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-neutral-950 px-4 py-8 text-neutral-50 sm:px-6 lg:px-8">
      ...
      <div className="pointer-events-none absolute right-[-12rem] top-[20rem] h-96 w-96 rounded-full bg-violet-400/5 blur-3xl" />
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full border border-violet-300/25 bg-violet-300/5 px-2.5 py-1 text-[10px] font-semibold tracking-[0.16em] text-violet-100">{copy.smart}</span>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Link href={`/hotel-factory/advanced/new?lang=${lang}`} className="rounded-2xl border border-violet-300/25 bg-violet-300/5 px-3 py-2 text-xs font-semibold text-violet-100 transition hover:border-violet-300/45">{copy.advanced}</Link>
