...
const COPY = {
  bg: {
    steps: ["Хотел", "Стаи и езици", "Екипи", "Услуги", "Контакти", "Преглед"],
    ...
    contactsTitle: "Как гостите да се свързват с екипите?", contactsHelp: "Добави телефон, WhatsApp и/или email за избраните екипи. Полетата са по избор и се използват като guest-facing контакти.",
    phone: "Телефон", whatsapp: "WhatsApp", email: "Email", configuredContacts: "Екипи с контакт",
    reviewTitle: "Преглед на тестовия Hub", reviewHelp: "Нищо няма да стане LIVE. Първо създаваме безопасен draft.",
    hotel: "Хотел", rooms: "Стаи", teams: "Екипи", services: "Услуги", contacts: "Контакти", validate: "Провери конфигурацията", validating: "Проверка…",
  ...
  en: {
    steps: ["Hotel", "Rooms & languages", "Teams", "Services", "Contacts", "Review"],
    ...
    contactsTitle: "How can guests contact the teams?", contactsHelp: "Add a phone number, WhatsApp and/or email for the selected teams. All fields are optional and are used as guest-facing contacts.",
    phone: "Phone", whatsapp: "WhatsApp", email: "Email", configuredContacts: "Teams with contact",
    reviewTitle: "Review your test Hub", reviewHelp: "Nothing becomes LIVE. We create a safe draft first.",
    hotel: "Hotel", rooms: "Rooms", teams: "Teams", services: "Services", contacts: "Contacts", validate: "Check configuration", validating: "Checking…",
...
export default function HotelManagerOnboardingWizard({ lang }: { lang: ControlPlaneLang }) {
  ...
  }), [hotelSlug, hotelName, countryCode, country, selectedLanguages, roomCount, roomMode, rangeStart, rangeEnd, roomList, selectedDepartments, selectedServices, departmentContacts]);
  ...
  function canAdvance() {
    ...
    if (step === 4 && selectedDepartments.some((id) => {
      const contact = departmentContacts[id];
      return contact.phone.trim().length > 160 || contact.whatsapp.trim().length > 160 || contact.email.trim().length > 320 || !validEmail(contact.email);
    })) return false;
    return true;
  }
  ...
  return <section className="relative overflow-hidden rounded-[2rem] border border-cyan-300/15 bg-neutral-900/90 p-5 shadow-[0_30px_120px_rgba(6,182,212,0.08)] backdrop-blur sm:p-7">
    ...
    <div className="relative">
      ...
      <div className="mb-7 grid gap-2 sm:grid-cols-3 lg:grid-cols-6">{copy.steps.map((label, index) => <div key={label} className={`rounded-2xl border px-3 py-3 text-xs transition ${index === step ? "border-cyan-300/50 bg-cyan-300/10 text-cyan-100 shadow-[inset_0_0_24px_rgba(34,211,238,.06)]" : index < step ? "border-emerald-400/20 bg-emerald-400/5 text-emerald-200" : "border-white/5 bg-black/20 text-neutral-500"}`}><span className="mr-2 font-mono">{String(index + 1).padStart(2, "0")}</span>{label}</div>)}</div>
      ...
      {step === 4 && <Panel title={copy.contactsTitle} help={copy.contactsHelp}>
        <div className="space-y-4">
          {selectedDepartments.map((departmentId) => {
            const department = DEPARTMENTS.find((item) => item.id === departmentId) || DEPARTMENTS[0];
            const contact = departmentContacts[departmentId];
            return <section key={departmentId} className="rounded-3xl border border-white/5 bg-black/20 p-4 sm:p-5">
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-3">
                <label className="text-sm text-neutral-300">{copy.phone}<input inputMode="tel" value={contact.phone} onChange={(e) => patchContact(departmentId, { phone: e.target.value })} placeholder="+359 2 123 4567" maxLength={160} className={field} /></label>
                <label className="text-sm text-neutral-300">{copy.whatsapp}<input inputMode="tel" value={contact.whatsapp} onChange={(e) => patchContact(departmentId, { whatsapp: e.target.value })} placeholder="+359 88 123 4567" maxLength={160} className={field} /></label>
                <label className="text-sm text-neutral-300">{copy.email}<input type="email" value={contact.email} onChange={(e) => patchContact(departmentId, { email: e.target.value })} placeholder="reception@example.com" maxLength={320} className={field} /></label>
              </div>
            </section>;
          })}
        </div>
      ...
      {step === 5 && <Panel title={copy.reviewTitle} help={copy.reviewHelp}>
        <div className="grid gap-3 sm:grid-cols-2"><Metric label={copy.hotel} value={hotelName} /><Metric label={copy.rooms} value={String(roomCount)} /><Metric label={copy.languages} value={languageLabels.join(", ")} /><Metric label={copy.teams} value={teamLabels.join(", ")} /><div className="sm:col-span-2"><Metric label={copy.services} value={serviceLabels.length ? serviceLabels.join(", ") : "—"} /></div><div className="sm:col-span-2"><Metric label={copy.contacts} value={`${configuredContactCount} / ${selectedDepartments.length}`} /></div></div>
