...
export default async function HotelScannerPage({
  ...
  if (!authority) {
    const next = normalizeAdminNextTarget(`/hotel-scanner?lang=${lang}`, lang);
        ...
        <header className="rounded-[2rem] border border-cyan-300/15 bg-neutral-900/80 p-6 shadow-[0_30px_100px_rgba(6,182,212,0.06)] backdrop-blur-xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Link href={`/hotel-factory/new?lang=${lang}`} className="rounded-2xl border border-white/10 bg-black/20 px-3 py-2 text-xs font-semibold text-neutral-300 transition hover:border-white/20">{copy.smartSetup}</Link>
