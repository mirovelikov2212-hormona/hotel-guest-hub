...
import { getRuntimeCellFleetSnapshot } from "@/lib/server/runtime-cell-control-plane";
...
const COPY = {
  bg: {
    ...
    subtitle: "Логическо разпределение на всички hotel tenants",
  ...
  en: {
    title: "Runtime cells",
    subtitle: "Logical partitioning of every hotel tenant",
    ...
    <main className="min-h-screen bg-neutral-950 px-4 py-8 text-neutral-50 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-3xl border border-neutral-800 bg-neutral-900 p-6">
          ...
          <p className="mt-5 rounded-2xl border border-cyan-300/20 bg-cyan-300/5 px-4 py-3 text-sm text-cyan-100">
            {copy.invariant}
          </p>
        </header>
            ...
            <article key={cell.id} className="rounded-3xl border border-neutral-800 bg-neutral-900 p-5">
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
                <div className="rounded-2xl bg-neutral-950 p-3">
                  <p className="text-neutral-500">{copy.hotels}</p>
                  <p className="mt-1 font-semibold">{cell.hotelCount}</p>
                </div>
                <div className="rounded-2xl bg-neutral-950 p-3">
                  <p className="text-neutral-500">{copy.capacity}</p>
                  <p className="mt-1 font-semibold">{cell.hotelCount}/{cell.maxHotels}</p>
                </div>
                <div className="rounded-2xl bg-neutral-950 p-3">
                  <p className="text-neutral-500">{copy.p95}</p>
                  <p className="mt-1 font-semibold">≤ {cell.desiredMaxP95Ms} ms</p>
                </div>
                <div className="rounded-2xl bg-neutral-950 p-3">
                  <p className="text-neutral-500">{copy.target}</p>
                  <p className="mt-1 truncate font-semibold">{cell.routingTargetKey}</p>
                </div>
              </div>
              ...
              <details className="mt-5 rounded-2xl border border-neutral-800 bg-neutral-950">
                <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-neutral-200">
                  {copy.hotels} ({cell.hotelCount})
                </summary>
                <div className="border-t border-neutral-800 px-4 py-2">
                  {cell.hotels.length ? cell.hotels.map((hotel) => (
                    <div key={hotel.hotelId} className="flex flex-col gap-1 border-b border-neutral-900 py-3 last:border-b-0 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm font-medium text-neutral-100">{hotel.name}</p>
                        <p className="text-xs text-neutral-500">{hotel.slug}{hotel.publicSlug && hotel.publicSlug !== hotel.slug ? ` · ${hotel.publicSlug}` : ""}</p>
                      </div>
