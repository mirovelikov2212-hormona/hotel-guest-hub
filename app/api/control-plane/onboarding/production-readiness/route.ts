...
import { assessFactoryProductionReadiness } from "@/lib/server/factory-production-readiness";
...
function mapFactoryError(error: unknown) {
  ...
  if (message.includes("P2_6_1_FACTORY_ADMIN_FORBIDDEN")) return { status: 403, code: "forbidden" };
  if (message.includes("P2_6_1_IDEMPOTENCY_CONFLICT")) return { status: 409, code: "conflict" };
  if (message.includes("P2_6_1_")) return { status: 400, code: "production_readiness_gate_failed" };
...
export async function POST(req: NextRequest) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) return jsonResponse({ ok: false, error: "payload_too_large" }, 413);
  ...
  try {
    ...
    console.error("Control Plane Production readiness assessment failed", error);
