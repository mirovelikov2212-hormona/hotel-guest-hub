...
import {
  ...
  updateStaffStandardAuthoringSourceText,
...
function safeError(error: unknown) {
  ...
  return /^[A-Z0-9_]+(?::[A-Z0-9_,.-]+)?$/.test(message)
    ? message
    : "STAFF_STANDARD_AUTHORING_FAILED";
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    const authoring = await listStaffStandardAuthoring({ hotelSlug });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!body) {
      return NextResponse.json(
        { ok: false, error: "STAFF_STANDARD_AUTHORING_BODY_REQUIRED" },
    ...
    if (action === "create_draft") {
      const result = await createStaffStandardAuthoringDraft({
        hotelSlug,
    ...
    if (action === "update_source_text") {
      const result = await updateStaffStandardAuthoringSourceText({
        hotelSlug,
    ...
    if (action === "save_proposal") {
      const result = await saveStaffStandardAuthoringProposal({
        hotelSlug,
        authoringId: body.authoringId,
    ...
    if (action === "publish") {
      const result = await publishStaffStandardAuthoring({
        hotelSlug,
        authoringId: body.authoringId,
      });
      return NextResponse.json({ ok: true, result }, { headers: NO_STORE_HEADERS });
    ...
    return NextResponse.json(
      { ok: false, error: "STAFF_STANDARD_AUTHORING_ACTION_INVALID" },
      { status: 400, headers: NO_STORE_HEADERS },
    );
  } catch (error) {
    console.error("staff standard authoring action failed", error);
    return NextResponse.json(
      { ok: false, error: safeError(error) },
      { status: status(error), headers: NO_STORE_HEADERS },
    );
